...
Implement and validate in this dependency order. Version 0.2.7 includes the July 20 reader/detail feedback follow-up. The current gate passes 252 JVM tests across 42 suites plus lint and both APK assemblies. The reported reader-ownership race, Exit, and tutorial timing/dismissal are target-device validated; broader format/responsive checks remain.
...
5. [x] Establish the Readium audiobook playback foundation: Readium 3.0.2 media `AudioNavigator` with its ExoPlayer adapter, a Media3 foreground `MediaSessionService`, media-playback/notification permissions, an application-scoped controller, local M4B and other recognized audio opening, and an explicit task-removal survival policy. Focused format tests and compilation pass; device validation remains pending.
   - [ ] Finish screen-to-controller lifecycle wiring and validate notification metadata/actions, lock-screen/headset/Bluetooth controls, audio focus/noisy handling, progress/offline/Preview semantics, service restoration, and explicit Close-to-stop behavior.
6. [ ] Add one app-global compact audiobook player above every destination, with no expandable or fullscreen player. Match only the official Audiobookshelf small-player interaction and layout as closely as appropriate: compact cover, title/author, progress, play/pause, timed rewind/forward, and an explicit Close action without hiding normal app navigation. Preserve progress sync/offline behavior and explicitly settle Preview minimization/close semantics before implementation.

...
- [ ] Next priority: make Android system audiobook controls show and execute Back 10 / Play-Pause / Forward 30 in the pull-down player and lock screen while retaining chapter navigation in the app player.
  - [x] Keep both playback engines on the correct timed increments: direct Media3 uses 10/30 seconds, and local Readium is explicitly configured to the same values.
  - [x] Preserve the completed preparation repair: Media3 remains at the Readium 3.0.2-compatible 1.4.1 line with bounded binding/preparation and failure-safe cleanup. The earlier Media3 1.9.0 attempt exposed the desired player-command buttons but broke local and streamed audiobook preparation.
  - [x] Diagnose why commit `e452d24` still leaves only Play/Pause on API 33+: Android SystemUI derives controls from the platform `PlaybackState`, not `MediaNotification.Provider`; Media3 1.4.1 exports custom-layout buttons there only when they use custom `SessionCommand`s. The current Back/Forward buttons use `Player.COMMAND_SEEK_BACK/FORWARD`, so they are omitted, and filtering previous/next leaves slots 2 and 3 empty. Existing tests exercise helper/provider output rather than the real platform session.
...
- [x] Move the Book Detail reading-status and reading-percentage display below the cover and above the star rating
- [x] When the author is tapped in Book Detail, open the author screen for that author; physical validation passes
...
- The reader tutorial work order is implemented. User-confirmed physical validation now covers the reader/media/layout/accessibility/responsive/theme/timing/resume/Preview-isolation/offline/edge-state matrix and remaining supported-media checks. Offline RAR/7z extraction and direct OIDC/SSO remain explicitly deferred.
...
- [x] Complete Phase 1 OIDC/SSO compatibility: the Open server sign-in WebView is implemented, continuously verifies the asynchronous server session, automatically closes after authentication, resumes the pending destination, and is user-validated. Native AppAuth remains a separate follow-up after BookOrbit PR #554 or equivalent mobile-redirect support is deployed. See [OIDC / SSO Authentication](docs/oidc-authentication.md). Tracked in [GitHub issue #4](https://github.com/van-geaux/lagrange-reader/issues/4).

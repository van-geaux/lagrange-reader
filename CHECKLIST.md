...
Implement and validate in this dependency order. The prior extraction-root/appassets-root/SVG candidates still failed on the Samsung Galaxy S24. The user supplied three valid, unencrypted sample EPUBs—`01 - your name.epub`, `04 - Zero Damage Sword Saint_ I Enrolled in a Magic School and Wound Up in a Contract with the Demon Queen_ Volume 4.epub`, and `05 - Overlord, Vol. 5.epub`—and Calibre renders their covers/images. Exact-sample WebView 149 emulator diagnostics also decode and lay out every referenced image, showing that the emulator path is healthy while the S24 path differs. The new production path removes data-document and shared-cache variables through `EpubWebViewAssetSession`: unique no-store appassets virtual chapter documents loaded with `loadUrl`, escaped `<base>` elements preserving original chapter directories, per-book root mounting at `/`, `LOAD_NO_CACHE` in visible/hidden WebViews, disabled broad file/content access, and a four-document byte-array cap. The full gate passes 215 JVM tests across 35 suites with zero failures/errors/skips plus `lintDebug`, `assembleDebug`, `assembleDebugAndroidTest`, and targeted real-WebView emulator test 1/1.

1. [ ] Install `app/build/outputs/apk/debug/app-debug.apk` on the Samsung Galaxy S24 and retest all three supplied EPUBs plus a mixed cover-missing/later-images-working title through normal Read, nonlocal Preview after relogin, and local download where applicable. Confirm every cover/illustration renders and remains page-constrained, visible/hidden pagination stays stable, and CBZ/CBR remains unaffected. Do not advance other work or mark the regression fixed until this exact phone retest passes.
...
- EPUB remains the representative reader path. The new virtual no-store document path passes exact-sample emulator diagnostics and the full gate, but the Samsung Galaxy S24 retest remains the highest-priority next task.

...
Implement and validate in this dependency order. The extraction-root fix still failed on the target device: `your name.` and `Overlord, Vol. 5` show no images, some EPUBs miss the cover but show later images, and `Zero Damage Sword Saing Vol. 4` shows all images as a control. The new code mounts the dedicated per-book extracted root at appassets `/` rather than `/epub/`, keeping root-relative resources on appassets and inside the current book. Reader CSS applies containment sizing to normal `img` elements without forcing `height:auto` on outer full-page SVG wrappers. The expanded WebView fixture covers parent-relative and root-relative `img` plus a full-page SVG `xlink:href`, and `EpubPaginationTest` guards the SVG CSS. The full gate passes 214 JVM tests across 35 suites with zero failures/errors/skips plus `lintDebug`, `assembleDebug`, and `assembleDebugAndroidTest`; physical confirmation remains open.

1. [ ] Install `app/build/outputs/apk/debug/app-debug.apk` and physically verify the new appassets-root/SVG compatibility fix. Retest `your name.`, `Overlord, Vol. 5`, a title whose cover was missing while later images worked, and the fully working `Zero Damage Sword Saing Vol. 4` control. Cover parent-relative, root-relative, and full-page SVG images in opened/local and nonlocal Preview flows after relogin. Confirm images remain page-constrained, visible/hidden pagination stays stable, CBZ/CBR images remain unaffected, and broad WebView file/content access stays disabled. Do not advance other work or mark the regression fixed until this passes.
...
- EPUB remains the representative reader path. The prior extraction-root fix failed physically; the appassets-root/SVG compatibility step passes automation but its four-sample target-device retest remains the highest-priority next task.

...
- [ ] **Highest priority:** make Android system audiobook controls show and execute Back 10 / Play-Pause / Forward 30 in the pull-down player and lock screen while retaining chapter navigation in the app player.
  ...
  - [ ] Define stable Back-10 and Forward-30 custom `SessionCommand`s and session-command `CommandButton`s. Grant both commands in `MediaSession.Callback.onConnect`, handle them in `onCustomCommand` through the active player's bounded `seekBack()`/`seekForward()`, retain standard seek-command availability and previous/next filtering, and reuse the same buttons in the notification provider for API 26-32.
  - [ ] Replace the false-positive coverage with focused unit tests plus real-session instrumentation. On API 33+, query the actual `MediaSession` platform token, assert ordered Back-10/Forward-30 `PlaybackState` custom actions and icons, invoke them, and assert exact bounded -10/+30 position changes. Retain compact-index/provider coverage for API 26-32 and exercise both local Readium and direct streamed Media3 command sets.
  - [ ] Compile every affected source set, run focused tests and the complete unit/lint/APK gate, and build a fresh `app/build/outputs/apk/debug/app-debug.apk`. Do not mark this item complete until a physical API 33+ device shows and correctly executes all three pull-down and lock-screen controls for one local and one streamed audiobook, including boundaries, paused/background/task-relaunch states, and headset/Bluetooth behavior where available; spot-check the API 26-32 fallback.

...
Implement and validate in this dependency order. Version 0.2.5 implements option A: a compact primary-colored `Reading · n%` or `Read · n%` line directly above the fixed action row. It uses finite canonical 0–100 progress, shows at most two decimals, accepts opened 0%, suppresses unread-reset 0%, and uses status-only text for unknown percentage. BrowserState overrides stale detail progress so Mark as unread removes the line immediately. The gate passes 235 JVM tests across 39 suites plus lint and both APK assemblies.
...
- The next implementation is the still-unfinished Suwayomi lightweight reader chrome, followed by its exact one-second tutorial. Physical detail/action-row and comic checks remain in the device-validation queue.

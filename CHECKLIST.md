...
Implement and validate in this dependency order. Commit `4854b0b` still failed on the Samsung Galaxy S24. The added `sample/01 - Overlord, Vol. 1.epub` shows a failed opening cover but a working ordinary chapter-3 image. Inspection isolates the failed opening pages in `your name.`, Overlord Vol. 1, and Overlord Vol. 5 as the supplied set's only bitmap-only SVG pages; fully working `Zero Damage Sword Saint Volume 4` has no SVG page. Every sample correctly declares a JPEG EPUB3 `cover-image`, so missing OPF cover metadata is excluded, and there is no Readium/`publication.cover()` dependency. `normalizeBitmapOnlySvgPages` now runs in `styleEpubHtml` before pagination and converts only a single-raster-image/no-vector-primitives SVG into an eager/sync `img`, preserving source/id/class/accessibility text. Strong `bookorbit-svg-raster` CSS supplies block layout, auto dimensions, page max bounds, centering, and contain fitting; genuine vector SVG and ordinary images remain untouched. The permanent real-WebView test passes 1/1, and the full gate remains 215 JVM tests across 35 suites with zero failures/errors/skips plus `lintDebug`, `assembleDebug`, and `assembleDebugAndroidTest`.

1. [ ] Install `app/build/outputs/apk/debug/app-debug.apk` on the Samsung Galaxy S24 and retest `your name.`, Overlord Vol. 1, Overlord Vol. 5, and `Zero Damage Sword Saint Volume 4` through normal Read, nonlocal Preview after relogin, and local download where applicable. Confirm normalized bitmap-only SVG covers render and fit the page, Overlord Vol. 1 chapter 3 still renders its ordinary image, genuine SVG/ordinary images remain intact, and CBZ/CBR stays unaffected. Do not advance other work or mark the regression fixed until this exact phone retest passes.
...
- EPUB remains the representative reader path. Bitmap-only SVG cover normalization passes the permanent emulator test and full gate, but the Samsung Galaxy S24 retest remains the highest-priority next task.
...
- The immediate order is the July 19 work order above: physical EPUB fix verification, single-row detail actions, conditional detail progress after placement confirmation, then Suwayomi-inspired reader chrome/tutorial and remaining validation.

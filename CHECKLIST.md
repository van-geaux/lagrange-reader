...
Implement and validate in this dependency order. Version 0.2.4 completes the responsive book-detail action row. It is fixed at 46 dp high and never wraps or scrolls. Read/Preview remain labeled; nonlocal Download/Retry/Cancel keeps an inline icon slot even when disabled; local Delete plus Update/Cancel update lives in More; typography-aware measurement moves Mark as read/unread into More when necessary. Only Read/Preview become weighted at extreme widths. The gate passes 229 JVM tests across 38 suites plus lint and both APK assemblies.
...
- The immediate implementation order is: confirm detail progress placement with the user, implement conditional detail progress, then complete the still-unfinished Suwayomi lightweight chrome/tutorial. Physical action-row and comic checks remain in the device-validation queue.

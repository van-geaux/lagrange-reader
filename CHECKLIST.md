...
Implement and validate in this dependency order. The latest target-device result found that EPUB chapter text opens but embedded images do not render, while CBZ/CBR images continue to work. The code fix now passes the full dedicated extraction root through EPUB parsing and mounts that root in `WebViewAssetLoader`, allowing parent-relative assets outside the OPF package directory while preserving the archive boundary. The extraction root, package file, and chapter paths are canonicalized; package/chapter paths escaping the extraction root are rejected. Updated JVM/WebView coverage crosses from `OEBPS/Text/chapter.xhtml` to `../../Images/cover.png`. The combined gate passes 213 JVM tests across 35 suites with zero failures/errors/skips plus `assembleDebug`, `assembleDebugAndroidTest`, and `lintDebug`; physical confirmation remains open.

1. [ ] Install `app/build/outputs/apk/debug/app-debug.apk` and physically verify the implemented EPUB extraction-root fix with opened/local and nonlocal Preview samples after relogin, including nested and package-boundary-crossing parent-relative resources. Confirm images render and remain page-constrained, visible/hidden pagination stays stable, and CBZ/CBR image rendering remains unaffected. Do not mark the regression fixed from automated coverage alone.
...
- EPUB remains the representative reader path. The extraction-root code fix and automated gate pass, but target-device confirmation of embedded images is the highest-priority next task.

...
- [x] [#42](https://github.com/van-geaux/lagrange-reader/issues/42): cached series Previous/Next now replaces remembered Book Detail state by target book/file identity, so forward and reverse navigation work without an online detail response; cached series screens also include books from every cached library. Automated coverage passes and the user confirmed the fix works offline on-device.

...
Currently Reading cards now expose a visible overflow action and the same menu on long-press. Removing a title calls BookOrbit's full reading-state reset endpoint, then clears matching local queued, last-synced, exact EPUB resume, active-reader, detail, snapshot, and Room-catalog progress so stale local work cannot restore it. On Deck now requires at least one completed book in a series, selects the first unread volume, and hides the shelf entry while that volume is actively being read. The reset flow and revised shelf behavior require target-server/device validation.

...
Latest device feedback confirms swipeable tiles, Series navigation, wrapped genre/tag metadata, grouped metadata cards, text wrapping, reader options, cache-first opening, exact jumps, thumbnail warnings, rapid-scroll cancellation, detail loading, and airplane-mode behavior. A manga sample is now available: CBZ was previously implemented but target content currently reaches the unsupported-format path, so comic format detection/routing is reopened as a regression investigation. CBR needs separate validation and may require RAR archive support. Audiobook testing remains deferred until a representative sample is available. Direct OIDC/SSO remains open/deferred.
...
The next target-device work order is to reconcile the still-open detail immediately after Delete local, restore cached Local books thumbnails including offline, make the main Options destination open from book details, restore genuinely in-progress titles to Currently reading, and aggregate Home shelves across every library on the connected server while retaining selected-library scope in Libraries/Browse. Comic work is reopened now that manga samples exist: diagnose the CBZ unsupported-format regression, restore CBZ routing/reading, and define CBR handling according to available RAR tooling and the supplied samples.

The current verification pass completes 170 JVM tests across 28 suites with zero failures/errors/skips; `:app:testDebugUnitTest :app:lintDebug :app:assembleDebug :app:assembleDebugAndroidTest` passes. Fresh APK: `app/build/outputs/apk/debug/app-debug.apk`.

Target-device follow-up confirms Read/Preview labels, live download progress, long-title expansion, series-index presentation, multi-selection, and correct layout-derived EPUB whole-book progress. Download/Delete local labels, immediate open-detail reconciliation after download completion, removal of the misleading Tag tap affordance, dismissible Home messages, and completed-only Recently read are implemented. Delete-local completion still needs the equivalent immediate detail-state reconciliation. Remaining EPUB validation covers timing/stability of page measurement on representative devices. Direct OIDC/SSO authentication remains deferred. Audiobook testing remains deferred without a sample; comic validation is reopened with the available manga samples.

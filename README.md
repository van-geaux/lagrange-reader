...
Working prototype. The app shell, API wiring, local download tracking, sync queue, explicit authenticated-session bootstrap via `/api/v1/auth/me`, sign-out/session reset behavior, and EPUB/PDF/audio/CBZ reading paths are in place. The remaining work is concentrated around end-to-end offline verification, reader quality, and release hardening.

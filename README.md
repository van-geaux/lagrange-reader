...
Book details now use compact horizontally swipeable action tiles with persistent labels and selective icons, link the series title to Series details, wrap Genre and Tag chips in separate groups, and organize lower metadata into compact Publication, Identifiers, and Library/file cards. Tapping the detail cover opens a full-screen viewer that closes when the cover is tapped again or Android Back is pressed. Compose instrumentation regression coverage exercises the action area, metadata content, cover-viewer tap dismissal, and Series navigation.

...
Options now has native grouped Interface and Data sections. Interface persists lock-current-orientation, haptic feedback, one flat app Theme list (Follow system/Light/Charcoal/Warm black/OLED black), default opening screen (Home/Library/Local books), and Reduce motion settings. Data persists downloads-over-cellular (Always/Never/Ask), reports downloaded/disposable-cache sizes, offers confirmed Clear cache that preserves downloads and downloaded-book metadata, controls current background cover work (and future metadata work) with Any network/Wi-Fi only/Disabled, and confirms local-copy deletion by default. Target-device feedback validates lock-current-orientation, the default opening screen, Reduce motion, downloads-over-cellular behavior, storage/cache clearing, and local-copy delete confirmation. The haptic perceptibility fix is implemented: when enabled, Compose requests use Android `CONFIRM` on API 30+ or `VIRTUAL_KEY` on API 26-29 through `View.performHapticFeedback`, preserving system haptic settings; enabling the preference confirms immediately, while disabled mode remains a no-op. Target-device validation is pending. Explicit coverage remains limited to Options switch/selection rows and supported Foundation long presses; broader navigation, ordinary book taps, and page-turn coverage remains an open design decision. App-theme and background-network-policy device validation also remain open.
...
The latest Achievements verification passes 207 JVM tests across 34 suites with zero failures/errors/skips; `testDebugUnitTest`, `lintDebug`, `assembleDebug`, and `assembleDebugAndroidTest` pass. Fresh APK: `app/build/outputs/apk/debug/app-debug.apk`.

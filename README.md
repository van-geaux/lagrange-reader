...
Book details now use compact horizontally swipeable symbol actions on transparent cards, link the series title to Series details, wrap Genre and Tag chips in separate groups, and organize lower metadata into compact Publication, Identifiers, and Library/file cards. Long titles are constrained to five rows with an expand/collapse affordance, and series name/index metadata remains visible as separate rows. Tapping anywhere in the full-screen cover viewer dismisses it, as does Android Back. Library grids support multi-book selection for bulk read/unread actions, and genre/tag navigation can open filtered book or Series results when the server exposes the matching filter contract. Compose instrumentation regression coverage exercises the action area, metadata content, cover-viewer dismissal, and Series navigation.

Latest device feedback confirms swipeable tiles, Series navigation, wrapped genre/tag metadata, grouped metadata cards, text wrapping, reader options, cache-first opening, exact jumps, thumbnail warnings, rapid-scroll cancellation, detail loading, and airplane-mode behavior. Audiobook and CBZ testing remains deferred until representative samples are available. OIDC remains an open investigation because the provider option is not currently surfaced in the app even when enabled on the server.

Follow-up detail-action feedback is now recorded for the next implementation pass: keep Read and Preview labels beside clearer icons, use an unmistakable Download icon, and expose active download progress/status directly from book details. The full-screen cover viewer, long-title expansion, and series-index layout are confirmed good.

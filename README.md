...
The first browser/local-state work-order step is implemented: Delete local immediately changes the still-open detail action back to Download, refreshes Local books, and updates Room catalog plus legacy snapshot `localPath` state; Local books can recover cover, author, format, series, and version metadata from the latest cached rich detail when catalog/snapshot metadata is incomplete; and opening Options from a book detail now dismisses the retained detail so Options is visible. Physical-device validation remains pending for these three fixes. Next are restoring genuinely in-progress titles to Currently reading and aggregating Home shelves across every library on the connected server while retaining selected-library scope in Libraries/Browse. Comic work remains open now that manga samples exist: diagnose the CBZ unsupported-format regression, restore CBZ routing/reading, and define CBR handling according to available RAR tooling and the supplied samples.

The latest browser/local-state verification passes 171 JVM tests across 28 suites with zero failures/errors/skips, `lintDebug`, and `assembleDebugAndroidTest`. Fresh APK: `app/build/outputs/apk/debug/app-debug.apk`.

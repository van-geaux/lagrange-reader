...
Book details now use compact horizontally swipeable actions: Read and Preview retain visible labels beside clear Play/Visibility icons, while Download uses a clear download symbol. Long titles are constrained to five rows with expansion, series name/index metadata remains visible, and the full-screen cover viewer dismisses from any screen tap or Android Back. Library grids support multi-book selection with overlap-safe bulk read/unread actions and stale-selection pruning. Genre chips open fully paginated server-filtered Books or Series results; tags remain informational. Active per-file downloads expose byte progress, percentage/linear or indeterminate state, cancel guidance, and retry failure status, and completed/auth-interrupted downloads clear active state. The login screen retains native credentials and adds a `Use server sign-in` WebView path for server-hosted OIDC/SSO, sharing cookies and polling `auth/me`.

Latest device feedback confirms swipeable tiles, Series navigation, wrapped genre/tag metadata, grouped metadata cards, text wrapping, reader options, cache-first opening, exact jumps, thumbnail warnings, rapid-scroll cancellation, detail loading, and airplane-mode behavior. Audiobook and CBZ testing remains deferred until representative samples are available. The previously missing OIDC entry path is now surfaced through server sign-in; redirect and completion behavior still need target-server validation.

Target-device follow-up confirms Read/Preview labels, live download progress, long-title expansion, series-index presentation, and multi-selection. The next work order adds labels to Download/Delete local, reconciles an open detail immediately after download completion, resolves an empty tappable-Tag result, makes red Home messages swipe/close dismissible, replaces normalized EPUB `/1000` location with layout-derived whole-book pages, and restricts Recently read to completed titles. Embedded server sign-in currently opens BookOrbit's own login page before its configured SSO redirect; authentication UX direction remains pending. Audiobook and CBZ testing remains deferred until representative samples are available.

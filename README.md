...
EPUB, PDF, and comic readers keep the display awake while their reading screen is visible, then restore the prior device timeout behavior when the user leaves. Audiobook playback continues to allow normal screen sleep. EPUB keeps Android's native top status bar visible for accurate battery and network indicators while the bottom navigation bar remains immersive; status icon contrast follows the selected reader theme. EPUB options expose one visible Close action; tapping exposed book content or pressing Android Back dismisses options before Back exits the reader.

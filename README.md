...
Working prototype. The app shell, API wiring, local download tracking, sync queue, authenticated-session bootstrap, sign-out/session reset behavior, and EPUB/PDF/audio/CBZ reading paths are in place. The visible app brand is Lagrange with the subtitle “a BookOrbit reader”; the native browser uses a Plex-inspired Home/Libraries/More bottom bar, a top logo/search/profile bar, a library picker, a dedicated search layer, and an About destination. Library books use the adaptive poster-card grid shared by Series and Authors. The EPUB reader includes exact local in-chapter resume plus Compact, Comfortable, and Wide padding presets, with Comfortable as the default and repagination on change. Startup keeps the branded splash state through bootstrap and uses a branded loading fallback instead of a spinner; physical-device launch validation remains pending. Focused JVM coverage exercises coordinator bootstrap, cached offline fallback, post-login resume flows, progress restoration, and recoverable browser-load failures. Compose instrumentation coverage now includes setup, login recovery, live/loading browser states, and cached offline behavior. Other media-specific validation remains deferred until representative samples are available.

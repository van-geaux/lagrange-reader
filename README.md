...
Target-device follow-up confirms Read/Preview labels, live download progress, long-title expansion, series-index presentation, and multi-selection. Download/Delete local labels, immediate open-detail reconciliation after completion, and removal of the misleading Tag tap affordance are implemented. Remaining work includes red Home message dismissal, layout-derived EPUB whole-book pages, and restricting Recently read to completed titles. Direct OIDC/SSO authentication remains deferred. Audiobook and CBZ testing remains deferred until representative samples are available.

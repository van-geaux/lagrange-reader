...
- Follow-up feedback is implemented: detail actions retain compact spacing while showing Read/Preview labels beside clear icons and Download as an icon-only action; titles expand to five rows, series/index rows remain visible, multi-book selection supports bulk read/unread, and the cover viewer dismisses from any screen tap. Genre chips navigate to paginated filtered Books/Series results; tags remain informational. Embedded server sign-in now supports the OIDC/SSO path; target validation remains required. Audiobook and CBZ testing remain deferred without samples.
- The audited follow-up completes 156 JVM tests across 26 suites with zero failures, errors, or skips. Read/Preview labels and clear icons, Download progress/retry/cancel states, multi-select pruning/header layout, paginated genre navigation, and embedded server sign-in/OIDC polling are covered by the implementation. `lintDebug`, `assembleDebug`, and `assembleDebugAndroidTest` pass; the debug APK is at `app/build/outputs/apk/debug/app-debug.apk`. Physical-device/server validation remains required.
- Target-device follow-up confirms Read/Preview labels, live download progress, long-title expansion, series-index presentation, and multi-selection. Remaining defects: Download/Delete local need labels; the open detail does not reconcile after download completion; the tappable Tag result is empty; red Home messages persist until refresh; the EPUB footer still uses normalized `/1000`; and Recently read includes unfinished titles. Embedded server sign-in reaches BookOrbit's own login page before its configured SSO redirect, so authentication UX direction remains a product decision.
...
- Audiobook and CBZ testing is intentionally deferred until representative samples are available. OIDC/SSO is now surfaced through `Use server sign-in`; provider redirects and completion still require a real-server validation.
...
4. Verify the native username/password form appears with password visibility control, Change server, and `Use server sign-in`. Open server sign-in and, on an OIDC/SSO-enabled deployment, confirm the embedded flow shares cookies and returns after `auth/me` polling detects authentication.
...
9. Confirm Continue reading/Currently reading is the first book shelf, active progress is shown there even if the server read flag is also set, completed books are excluded from it, and Recently read books remains a separate history shelf.
   Pending follow-up: Recently read must contain only completed titles; every unfinished/in-progress book must remain excluded. Red Home message cards must disappear immediately after a horizontal swipe or close-button tap without requiring refresh.
...
16. Horizontally swipe the compact book-detail action controls and confirm Read and Preview show visible labels beside clear Play/Visibility icons while Download uses an unmistakable download symbol. Start a download and verify per-file byte progress, percentage/linear or indeterminate state, cancel guidance, retry failure status, and clearing after completion or authentication interruption. Preview must leave normal progress unchanged.
   Pending follow-up: Download and Delete local must also retain visible labels, and completion must update the still-open detail screen from Download to Delete local without navigation or refresh.
...
18. In a library grid, long-press or select multiple books, confirm the header does not overlap and stale selections are pruned, then apply bulk Mark as read and Mark as unread. Tap a Genre chip in Books and Series and confirm fully paginated filtered results scoped to the selected library or Series catalog; tags remain informational. Record any server-side query incompatibility.
   Pending follow-up: verify whether BookOrbit exposes a supported Tag filter. A Tag tap must never open a misleading empty result; either show verified matching results or remove its navigation affordance.
...
2. Current behavior uses a normalized whole-book location out of 1000. Pending follow-up must replace this with actual layout-derived current/total whole-book pages, recalculated when viewport, text size, margins, or chapter pagination changes; the footer must remain visible and unobstructed.

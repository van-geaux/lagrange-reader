...
- Target-device follow-up confirms Read/Preview labels, live download progress, long-title expansion, series-index presentation, and multi-selection. Download/Delete local labels, immediate open-detail reconciliation after completion, and removal of the misleading Tag tap affordance are complete. Remaining defects: red Home messages persist until refresh; the EPUB footer still uses normalized `/1000`; and Recently read includes unfinished titles. Native username/password is the current authentication flow.

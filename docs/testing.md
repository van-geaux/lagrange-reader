...
- Series detail pull-to-refresh reloads `seriesDetailLoader` and clears the refreshing indicator on both success and failure; Home, Libraries, and More remain visible on Series detail (issue #62). Automated coverage is complete, and the user confirmed the rebuilt APK works on-device.

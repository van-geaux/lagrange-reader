...
Open the grouped format's `View files` inspector and tap `Download` for one incomplete physical file. Verify that only that file transfers and sibling files remain idle. Repeat with cellular download confirmation enabled and verify confirming the prompt still downloads only the selected physical file. Debug APKs used for this procedure are 64-bit ARM (`arm64-v8a`)-only.

Open the grouped format picker and verify ordinary tap still selects the whole group. Verify the closed summary has no separate `Files` button. Long-press a grouped row or use its eye-shaped `View files` action, and verify a read-only list opens with every physical file in authoritative server order, its filename, size when known, local downloaded/not-downloaded status, and a per-file control: idle incomplete files show `Download`, active or queued files show `Downloading` with no download button, and completed files show `Delete`. Tap one idle file’s button and verify only that physical file is queued/downloaded; tap one completed file’s delete control and verify only that physical file is removed while sibling files remain downloaded. Capture active state immediately after enqueue, before a fast transfer can complete. While the group has active or queued members, tap the Book Detail X action and verify all active/queued files in that group are canceled and the status returns to the normal prompt to tap X to stop/cancel when a new download begins. Verify inspection does not change group selection. Repeat with EPUB, PDF, M4A, MP3, M4B, and a mixed-format book where available; formats must remain separate selectable groups.
...
- with a compatible local EPUB 3 media-overlay book, open the EPUB normally and tap the center area to show reader controls. Verify `Listen to narration` appears only for a valid local overlay; narration starts and highlights each sentence, and Android system media controls target it. At each fresh start, resume, and selected-sentence start, verify the reader reaches the active SMIL locator and shows its highlight before audio begins. Rotate while narration is playing and verify the player, speed, highlight, and playback survive. Long-press part of a sentence and verify `Play narration` appears before `Web search`, remains visible without opening overflow, and starts at the containing sentence; verify `Web search` is inside overflow. Open Speed and verify the shared audiobook controls/range/steps/format/persistence apply. Tap the rightmost X and verify playback stops immediately, the player closes and releases its viewport space, the highlight clears, and the EPUB progress row sits directly above the player while remaining above the visible Android navigation bar. Close/reopen and verify saved-fragment resume, then repeat offline. Ordinary reading must not auto-play or create audiobook progress/history; an overlay-free EPUB must not show Listen or Play narration;
...
- audiobook compact-player restoration, seeking, chapters, and speed; both the compact and full players expose the shared adjustable playback-speed control. Open Speed and verify a centered full-screen modal overlay appears with a grey/dimmed scrim. Verify its first row contains `−0.10`, `−0.05`, the current speed, `+0.05`, and `+0.10`. Verify the current speed always uses two decimal places, such as `1.20x`, `1.25x`, and `1.30x`. Tap each adjustment and verify it applies immediately, persists, keeps the overlay open, and remains bounded from `0.25x` through `3.00x`. Verify Close X, Android Back, and tapping outside the overlay dismiss it while retaining the latest speed.
- issue #169 full-player progress: in portrait and landscape, verify the overall seek label shows `Book progress · N%` and the current-chapter seek label shows `Chapter progress · N%`, with live whole-number percentages and no label when a duration is unavailable. Verify the compact player omits these percentage labels. The user confirmed this behavior on-device; no assistant-side connected-device instrumentation was available.
...
On a connected Android device or emulator, open Options and verify `Pause audiobook for audio interruptions` is enabled by default. Start both an online audiobook and a downloaded/local audiobook, then trigger a competing app or system sound that requests Android audio focus; verify playback pauses and resumes after the competing focus ends in both paths. Disable the option, change it while playback is active, and verify the active session follows the new setting; repeat the audio-focus interruption and verify playback does not pause when disabled. Open and close the notification shade without triggering a competing audio-focus request and verify playback continues. Ordinary WhatsApp message notifications may not request audio focus and therefore may not pause playback; WhatsApp calls and other competing media that request audio focus should pause playback when the option is enabled. The user-confirmed device validation covers the earlier integrated interruption behavior; the follow-up policy-refresh correction has automated verification only, and no new assistant-side device execution is claimed.
...
Verify password login, `/api/v1/auth/me` bootstrap, sign-out/session reset, session-expiry recovery, and pending-destination recovery. For explicit SSO, discover the configured providers, select a provider, complete the IdP flow in the WebView, verify exact callback interception and successful callback exchange, then confirm the pending destination resumes. Also test provider cancellation/error, state mismatch rejection, expired/replayed callbacks, token refresh, second-account login, and the generic `Open server sign-in` fallback. This is server-hosted OIDC in a WebView, not native AppAuth; native AppAuth requires separate deployed redirect and provider/device validation.

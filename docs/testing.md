...
- Target-device follow-up confirms Read/Preview labels, live download progress, long-title expansion, series-index presentation, and multi-selection. Download/Delete local labels, immediate open-detail reconciliation after completion, removal of the misleading Tag tap affordance, dismissible Home messages, and completed-only Recently read are complete. The EPUB footer still uses normalized `/1000`. Native username/password is the current authentication flow.

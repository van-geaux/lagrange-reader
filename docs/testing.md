...
- each active/trash annotation card's three-dots overflow menu and an equivalent long-press open the same menu; normal tap still opens Preview; verify edit note, change highlight color/style, copy selected text, and move to trash on active cards, and restore and permanently delete on trash cards, each destructive action gated behind a confirmation dialog; edit note/color-style/move-to-trash use the offline mutation queue and remain queued while offline, while restore and permanent delete are authenticated online actions whose errors are surfaced to the user;
- verify the menu's first action opens the selected book's detail screen, and its second action opens the same annotation Preview/location as tapping the card;
- when selecting EPUB text, verify the native horizontal Android selection toolbar retains Copy, Share, Web search, Highlight, and Highlight + Note, and that selection capture still occurs before annotation dialogs; a custom vertical replacement is intentionally not used;

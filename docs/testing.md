...
- In airplane mode, open cached series index 2, navigate Next to cached index 3, then Previous back to index 2 while online detail loading is unavailable. Verify each target replaces the visible detail state immediately. Open a cached series from a secondary-library book and verify cached books from primary and secondary libraries remain visible. The user confirmed this issue #42 flow works offline on-device.

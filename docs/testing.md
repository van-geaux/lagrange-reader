...
- Follow-up feedback changes the detail actions to symbol-only transparent controls with reduced padding, limits titles to five rows with expansion, keeps series/index rows visible, adds multi-book selection with bulk read/unread, and dismisses the cover viewer from any screen tap. Genre/tag filter navigation still needs target-server query validation. OIDC is not surfaced when enabled server-side; audiobook and CBZ testing remain deferred without samples.
...
- Audiobook and CBZ testing is intentionally deferred until representative samples are available. OIDC remains open because its provider option is not surfaced in the app even when enabled on the server.
...
16. Horizontally swipe the compact book-detail action controls and confirm symbol-only transparent buttons have compact padding and accessible descriptions. Use Read or Continue reading to enter the reader and verify Preview, Download/Delete local, and Cancel download states still work as applicable; Preview must leave normal progress unchanged.
   Follow-up validation must also confirm Read and Preview show visible labels beside clear icons, Download uses an unmistakable download symbol, and an active download exposes understandable progress/status from the detail screen.
...
18. In a library grid, long-press or select multiple books, confirm selected-state feedback, then apply bulk Mark as read and Mark as unread. Tap a Genre or Tag chip in Books and Series and confirm the corresponding filtered result list; record any server-side query incompatibility.

...
8. [ ] Add the current book's dynamic Mark as read/Mark as unread action to book details. Placement awaits design approval; the recommended candidate is a three-dot overflow at the end of the swipeable action row so reading-state management stays available without competing with Read, Preview, Download, or Delete local.

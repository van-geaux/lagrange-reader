...
Continuous-mode refinement: prefetch the two previous and two next pages around the visible range, and use vertical Swipe up/Menu/Swipe down tutorial regions; paginated mode retains LR/RL Previous/Menu/Next. Physical validation must cover long-book scroll-down/up smoothness, adaptive cache memory stability, prefetch/reuse, and tutorial layout/labels.

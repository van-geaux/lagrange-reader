...
2. [ ] Deliver Back 10 / Play-Pause / Forward 30 in the Android pull-down player and lock screen while retaining chapter navigation inside the app player.
   ...
   - [ ] Complete real API 33+ platform-session and physical-device validation: query the actual platform token, assert ordered Back-10/Forward-30 `PlaybackState` custom actions/icons, invoke both actions, and verify exact bounded -10/+30 position changes for local and streamed playback on pull-down and lock-screen surfaces, including boundaries, paused/background/task-relaunch states, headset/Bluetooth where available, and an API 26-32 fallback spot-check.

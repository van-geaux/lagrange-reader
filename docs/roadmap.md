...
1. [ ] **Highest priority:** deliver Back 10 / Play-Pause / Forward 30 in the Android pull-down player and lock screen while retaining chapter navigation inside the app player.
...
4. [ ] Move the Book Detail reading-status and reading-percentage display below the cover and above the star rating.
5. [ ] When the author is tapped in Book Detail, open the author screen for that author.
...
5. [ ] Add an audiobook-only Session history menu below the detail actions. Persist play/pause events locally with the book identity, event time, and exact playback timepoint; tapping an event seeks to that point. Keep this data device-only, preserve it across app updates, clear it when the configured server changes, and rely on Android uninstall behavior to remove it with app data.
6. [ ] Add one global Library card-size setting in Options. Make the current size Small and add Medium and Large; apply the selected size consistently across libraries and book/card types.
10. [x] Name every debug APK `Lagrange-debug-yyyymmddhhmm.apk` using the actual build datetime; `assembleDebug` retains the standard Gradle output and creates the timestamped handoff copy from the same binary.
9. [x] Keep the BookOrbit app bottom navigation (Home, Libraries, More) visible when opening Book Detail. Implementation and automated verification are complete; physical-device/UI validation remains pending.
10. [x] Investigate whether the app can detect a newer GitHub release tag, and if supported implement update notification behavior: on app open/reopen, when a newer release is detected, show an overlay containing the release notes and provide buttons for Acknowledge that opens the GitHub release link and Ignore that dismisses it. Automated implementation and focused verification are complete; physical/UI validation remains pending for dialog presentation/scrolling, Acknowledge link launch, Ignore/reopen behavior, and offline handling.

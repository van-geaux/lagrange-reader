...
The current reader/detail, stability, audiobook, cover-aspect/alignment, Readium-PDF, tutorial-label, Series-grouping, and profile-menu gate passes 265 JVM tests across 46 suites with zero failures/errors/skips; `testDebugUnitTest`, `lintDebug`, `assembleDebug`, and Android-test APK assembly pass. Connected coverage proves the generated three-page PDF opens as `Profile.PDF` with exactly three positions; JVM/compiled coverage also includes the poster/shelf cover-slot alignment contract, PDF direct/Preview routing, mixed cover ownership/enrichment and persistence, Room v2, compact audiobook controls/caching/Preview, crash recovery, Browser-first audio relaunch, revised reader interaction, grouping, and profile-menu structure. Target-device PDF UI checks, mixed-library cover alignment, audio relaunch, revised audiobook UI/Preview, and remaining media/device validation remain pending.

...
The July 19 EPUB compatibility work order below overrides the general phase sequence: no book-detail, reader-chrome, or remaining validation work advances until bitmap-only SVG cover normalization passes the Samsung Galaxy S24 EPUB retest.
...
The virtual no-store document candidate in commit `4854b0b` remained insufficient on the Samsung Galaxy S24: `your name.` and an Overlord volume still showed no images. The newly supplied `sample/01 - Overlord, Vol. 1.epub` narrows the behavior further: its opening cover fails while an ordinary `img` in chapter 3 loads. Its first spine item, `OEBPS/Text/titlepage-1.xhtml`, is a bitmap-only full-page SVG containing one `image xlink:href=../Images/cover.jpg`. Across supplied samples, the failed opening covers in `your name.`, Overlord Vol. 1, and Overlord Vol. 5 are the only bitmap-only SVG pages; fully working `Zero Damage Sword Saint Volume 4` has no SVG page.

All samples correctly identify a JPEG manifest item with EPUB3 `properties=cover-image` (Zero Damage also carries legacy metadata), so missing OPF cover metadata is not the cause. The app has no Readium dependency or `publication.cover()` API. Before pagination, `styleEpubHtml` now runs `normalizeBitmapOnlySvgPages`: an SVG is converted only when it has exactly one raster `image` and no vector primitives. The replacement ordinary `img` is eager/sync and preserves source, id, class, and accessibility text. Strong `bookorbit-svg-raster` CSS provides block display, auto dimensions, page max bounds, centered margin, and contain fitting. Genuine vector SVGs and ordinary images remain unchanged.

Permanent `EpubAssetLoadingInstrumentedTest` now proves the SVG element becomes an `IMG` with decoded `naturalWidth` and nonzero fitted width/height, alongside parent/root normal images and stylesheet routing. The focused real-WebView emulator test passes 1/1. The full gate remains 215 JVM tests across 35 suites with zero failures/errors/skips, `lintDebug`, `assembleDebug`, and `assembleDebugAndroidTest`. Samsung physical success remains unconfirmed.

Execute the current work in this order:

1. Install `app/build/outputs/apk/debug/app-debug.apk` on the Samsung Galaxy S24 and retest `your name.`, Overlord Vol. 1, Overlord Vol. 5, and `Zero Damage Sword Saint Volume 4`. Confirm bitmap-only SVG covers render and fit, Overlord Vol. 1 chapter 3 remains visible, ordinary/genuine-vector images are unchanged, and CBZ/CBR remains unaffected. Do not claim resolution or advance other work until this passes.

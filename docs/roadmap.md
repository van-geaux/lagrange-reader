...
## Current status — 2026-07-30 — behavior-neutral cleanup complete
...
- [x] Implemented and user-confirmed on-device issue [#12](https://github.com/van-geaux/lagrange-reader/issues/12): Book Detail, Series, Authors, Local books, Statistics, and Achievements now use the shared pull-down/swipe refresh lifecycle. Automatic/background synchronization remains silent; explicit refresh stays visible through completion, duplicate gestures are ignored, prior successful statistics/achievements content is retained across failed refreshes, and Local books refresh does not interrupt active transfers or destructive actions. Series and Authors now place the shared refresh layer around their complete content region, matching Home and Library and preventing indicator-background overflow. Main/unit/Android-test compilation, unit tests, lint, and APK assembly pass.
- [x] Implemented and user-confirmed on-device issue [#13](https://github.com/van-geaux/lagrange-reader/issues/13): continuous EPUB keeps the right-side seek rail enabled as a normalized 0–100% active-resource slider, seeking by the current Readium locator's resource progression even when continuous mode reports one page/item. Whole-book progress and paginated EPUB/PDF/comic page-index rails remain unchanged. Focused JVM coverage, full JVM/lint/APK verification, and device validation are complete.
- [x] Implemented issue [#16](https://github.com/van-geaux/lagrange-reader/issues/16): show server-missing books with a yellow `Missing!` cover overlay. After reconciliation, the current BookOrbit server catalog remains authoritative for visibility: books returned by the server stay visible even when their registered file is tagged/state `missing`, while stale local-cache-only books are removed; cache-first content may remain visible transiently during reconciliation. The missing state must be distinct from a locally deleted download, normal covers remain unchanged, and unavailable file actions are hidden except cancellation or local-copy cleanup where applicable. Regression coverage covers parser, cache, and action-state behavior.
- [x] Implemented issue [#14](https://github.com/van-geaux/lagrange-reader/issues/14): add user-selectable reader fonts. EPUB-only font controls use a grouped dropdown with Publisher default, System serif, System sans-serif, and System monospace, with a separate Accessibility fonts section for AccessibleDFA and OpenDyslexic. Users can import one active validated `.ttf`/`.otf` custom font, replace it, or remove it; imported fonts are applied only to a cached rewritten reader copy. The selected font persists per library and Publisher default preserves the book's own CSS; existing settings/resume behavior is preserved. The user confirmed the complete reader-font flow works on-device.
...
- Product direction selected: retain the existing service/progress model, use direct authenticated Media3 byte-range playback for connected audio to match BookOrbit's WebUI, retain Readium for explicit local audio, and keep lazy Readium page resources for connected comics.
...
- Completed in code: preserve BookOrbit's exact eight read-state wire values in the catalog model and every cache. Home and Library Recommended map reading/rereading to Currently reading, want_to_read to its own shelf, and read/skimmed to Recently read. On Deck is derived from series progression: a series with a Read volume contributes its first non-Read volume unless that candidate is Reading/Rereading. Standalone and unstarted series are excluded. Deterministic parser/shelf/cache/coordinator coverage passes, and target-device feedback confirms basic On Deck behavior; selected-library transitions and offline reopening remain.
...
2. Completed: replace the hamburger/drawer browser shell with a Plex-inspired native bottom-navigation layout. Libraries open at the top-level library view, with a library-change control at the top of that screen; More opens Series, Authors, and Local books, while Options and About are available from the profile menu.
...
4. Completed: the About destination now presents the real app description, independent/non-official BookOrbit relationship disclaimer, installed version/build, connected server, acknowledgements, and relevant links. Its original More-menu placement was later superseded by the profile-dropdown revision.
...
- [x] Restore series-progression On Deck after the exact-state pass incorrectly replaced it with On Hold: require a Read volume, choose the first non-Read volume in series order, and hide it while Reading/Rereading.
...
- [ ] Add and validate direct OIDC/SSO authentication after the provider and redirect contract are confirmed; deferred by current product decision, native username/password remains current.
...
6. Completed by user-confirmed physical validation: target-device, accessibility, large-text, responsive, orientation, theme, timing, resume/sync, Preview-isolation, offline, edge-state, and remaining supported reader/media checks. Offline RAR/7z extraction and direct OIDC/SSO remain explicitly deferred; release-overlay and Book Detail app-navigation physical validation are intentionally deferred.
...
4. [x] Establish persistent audiobook playback through a foreground Media3 `MediaSessionService`. Explicit local downloads retain Readium 3.0.2's media `AudioNavigator` and ExoPlayer adapter; connected audio uses direct Media3 byte-range playback against BookOrbit's serve route. The application controller samples progress every 1.5 seconds independently of screen/background ownership; Preview is isolated, explicit Close publishes final normal progress and stops the service, and Android 13 notification permission is requested on first playback.
5. [x] Add only an app-global compact player above the browser bottom navigation, also visible in separate Readium EPUB/comic activities. It exposes tappable cover-to-detail, title/author, elapsed/remaining seek slider, Replay 10, play/pause, Forward 30, BookOrbit `audioMetadata` chapters, 0.75/1/1.25/1.5/2Ã— speed, and Close; audiobook detail says Play. Metadata loads before preparation and persists through active-reader/detail caches. Authenticated online Read and Preview now use remote byte-range resources and remain progress-isolated where applicable; local/downloaded playback is unchanged. `CookieManager` clearing is main-thread confined. No expanded/fullscreen player exists.
   - [x] Replace the failed remote Readium asset path with BookOrbit-compatible direct Media3 streaming. The serve URL becomes a native media item, authenticated headers are resolved per data-source open, one 401/403 renews and retries, and no implicit complete audio copy is created. The compact controls and foreground session now operate a common Media3 Player across direct remote and Readium-local engines.
...
The current reader/detail, stability, audiobook, cover-aspect/alignment, Readium-PDF, tutorial-label, Series-grouping, profile-menu, orientation, fixed Library controls, and remote-media gate passes 280 JVM tests across 48 suites with zero failures/errors/skips; `testDebugUnitTest`, `lintDebug`, `assembleDebug`, and Android-test APK assembly pass. Connected/compiled coverage proves the generated three-page PDF opens as `Profile.PDF` with exactly three positions and includes direct authenticated Media3 audio ranges, one credential-renewal retry, legacy full-media cache pruning, plus lazy comic page publications that open without fetching pages and request only a selected page. Target-device remote-audio/comic, PDF UI, mixed-library cover alignment, audio relaunch, revised audiobook UI/Preview, and remaining media/device validation remain pending.
...
Main Options now exposes Reading configuration with a library-name dropdown. Each library owns a persisted profile for LTR/RTL direction, EPUB theme, text size, and margins; EPUB reader edits save back to that owning profile. EPUB, PDF, and comic launches carry the library ID, so logical edge taps/tutorial wording and direction-aware comic/PDF pager layout follow the correct profile. Returning from a reader refreshes the main preference view. The full gate passes 286 JVM tests across 49 suites with 0 failures/errors/skips, lint, debug APK assembly, and Android-test APK assembly; APK: `app/build/outputs/apk/debug/app-debug.apk`. Physical-device validation is complete.

### Per-library PDF and Comics layouts - 2026-07-22

Each library reader profile now stores separate PDF and Comics layout modes (`Paginated` or `Continuous`) and a 0-48 dp continuous page gap. Defaults are PDF Continuous/16 dp and Comics Paginated/16 dp. Options > Reading exposes independent format controls and disables the relevant gap slider in paginated mode. PDF maps the profile to Pdfium `Axis.HORIZONTAL`/`Axis.VERTICAL` plus native `pageSpacing`. Paginated comics retain Readium `ImageNavigator`; Continuous routes every supported local/remote comic to the bounded lazy vertical reader with nearby loading, a 64 MB source-page cap, viewport downsampling with a 16-million-pixel decoded bound, retry, resume/progress, LTR/RTL logical taps, menu tap, page slider, and Reduce motion behavior. The full gate passes 291 JVM tests across 50 suites with zero failures/errors/skips, lint, debug APK assembly, and Android-test APK assembly. APK: `app/build/outputs/apk/debug/app-debug.apk`. Physical-device validation of both PDF and Comics layout modes is user-confirmed complete.

### EPUB continuous-scroll layout - 2026-07-22

`LibraryReaderPreferences` now persists per-library `epubLayoutMode` (`Paginated`/`Continuous`), defaulting to Paginated for backward compatibility. Options > Reading adds an EPUB layout selector without a gap slider; PDF and Comics controls are unchanged. Readium EPUB `EpubPreferences` maps Paginated to `scroll=false` and Continuous to `scroll=true`. The full gate passes 291 JVM tests across 50 suites with zero failures/errors/skips, lint, debug APK assembly, and Android-test APK assembly; APK: `app/build/outputs/apk/debug/app-debug.apk`. Physical-device validation of mode switching, long documents, resume/progress, direction, accessibility, large text/theme/margins, and rotation is user-confirmed complete.

### Reader profile persistence and in-reader configuration - 2026-07-22

EPUB preference writes now persist the complete owning-library profile, preventing partial reader saves from resetting PDF/CBR-CBZ layout modes or page gaps. EPUB, PDF, and CBR/CBZ option sheets expose that book library's full Reading configuration without a library selector: EPUB/PDF apply and persist changes live, while CBR/CBZ rebuilds its layout and retains the current page. The visible label is now `CBR/CBZ layout`. Continuous remote CBZ Preview accepts chunked or unknown-`Content-Length` pages through a bounded read capped at 64 MB. Regression coverage is added/compiled; the full gate passes 291 JVM tests across 50 suites with zero failures/errors/skips, lint, debug APK assembly, and Android-test APK assembly. APK: `app/build/outputs/apk/debug/app-debug.apk`. Focused physical-device validation, including reader-profile persistence/scoping and chunked/unknown-length CBZ Preview pages, is user-confirmed complete.

### Bounded continuous CBR/CBZ bitmap cache - 2026-07-22

### Audiobook speed and Browser-first session restoration - 2026-07-22

Audiobook playback speed is now a global persisted value (0.75/1/1.25/1.5/2x), reapplied across books and compact-player close/reopen. Active sessions persist NORMAL or PREVIEW launch mode. Explicit Play from Book Detail stays on Browser, prepares in the compact player with a spinner, and autoplays when ready. App/task restoration follows the same Browser-first flow but restores the saved position and speed paused and never autoplays. Explicit Close clears the active-session marker, task/app termination leaves it for restart restoration, and a failed restore clears stale state. The full 294 JVM-test/50-suite gate passes with 0 failures/errors/skips, lint, debug APK assembly, and Android-test APK assembly; APK: app/build/outputs/apk/debug/app-debug.apk. Physical validation remains pending.
...
Cache smoothing now serves hits synchronously to avoid placeholder flashes, preserves known aspect ratios so evicted pages retain height while reloading, and deduplicates simultaneous visible/prefetch loads. The half-heap LRU keeps the whole decoded book when it fits and otherwise evicts least-recently-used pages; Android memory pressure trims/clears it and reader close clears all book-scoped cache state. The ±2 prefetch window and continuous Swipe up/Menu/Swipe down tutorial remain. Physical-device long-book smoothness and memory-stability validation is still required.

### New user feedback work order - 2026-07-22

1. [x] **Highest priority:** after an app update/reopen, prevent a stale Currently reading `BookSummary` from opening normal Play/Read at the wrong position before the full server catalog refresh completes. The verified diagnosis is that `AppCoordinator.openBook` can build reader state from the supplied stale summary, while `BookOrbitRepository.buildReaderState` consults only local queued/last-synced progress plus the EPUB locator rather than performing a fresh server book-progress read.
   - [x] **Acceptance criteria:** completed: normal online open calls `syncPendingProgress()` then authoritative `loadBookDetail()` before `buildReaderState`; offline normal open skips server calls and uses cached/local state; Preview skips server progress refresh and retains isolation; three `AppCoordinatorTest` regressions pass.
   - [x] **Status:** implementation and physical-device update/reopen validation are complete; the user confirmed the repaired resume behavior works.
2. [x] Deliver Back 10 / Play-Pause / Forward 30 in the Android pull-down player and lock screen while retaining chapter navigation inside the app player; user-confirmed physical validation is complete.
   - [x] Preserve the working engine behavior and preparation repair: direct Media3 and local Readium both seek by 10/30 seconds; explicit Media3 stays at the Readium 3.0.2-compatible 1.4.1 line with bounded preparation and cleanup. Media3 1.9.0 exposed modern player-command button preferences but broke audiobook preparation.
   - [x] Root cause the remaining API 33+ failure. SystemUI builds its controls from platform `PlaybackState`, so `MediaNotification.Provider` changes apply only before API 33. Media3 1.4.1 converts custom-layout buttons to platform custom actions only when they carry custom `SessionCommand`s; the current buttons carry `Player.COMMAND_SEEK_BACK/FORWARD`. With previous/next filtered, SystemUI receives only Play/Pause. The existing automated tests validate helper/provider output and never inspect the platform session.
   - [x] Keep Media3 1.4.1 and implement a narrow compatibility bridge: stable Back-10 and Forward-30 custom `SessionCommand`s, session-command `CommandButton`s, explicit `onConnect` authorization, and `onCustomCommand` dispatch to the active player's bounded `seekBack()`/`seekForward()`. Retain standard seek commands and previous/next filtering, and reuse the same buttons in the provider for API 26-32.
   ...
   - [x] Run affected-source-set compilation, focused tests, the full unit/lint/APK gate, and produce fresh debug and Android-test APKs.
   - [x] Complete real API 33+ platform-session and physical-device validation: query the actual platform token, assert ordered Back-10/Forward-30 `PlaybackState` custom actions/icons, invoke both actions, and verify exact bounded -10/+30 position changes for local and streamed playback on pull-down and lock-screen surfaces, including boundaries, paused/background/task-relaunch states, headset/Bluetooth where available, and an API 26-32 fallback spot-check. User-confirmed validation is complete.
   - Provider-only customization cannot repair API 33+; a Media3 upgrade requires a coordinated Readium migration; mapping timed seeks onto previous/next would misrepresent the controls to external clients.
3. [x] Add an editable five-whole-star control below the Book Detail cover for BookOrbit's authenticated per-user rating, distinct from decimal/aggregate metadata. Tap 1-5 to set and tap the selected star to clear. Writes are followed by an authoritative detail re-fetch; offline/cache fallback and optimistic rollback are implemented. The full 299-JVM-test/50-suite gate, lint, and both APK assemblies pass; physical target-device validation remains pending.
4. [x] Rename the book-detail reading-status action to `Mark as...` and show the complete server status list: Unread, Want to read, Reading, Rereading, On hold, Abandoned, Read, and Skimmed. User-confirmed physical validation shows each selection updates BookOrbit and local/cache state.
5. [x] Move the Book Detail reading-status and reading-percentage display below the cover and above the star rating.
6. [x] When the author is tapped in Book Detail, open the author screen for that author; user-confirmed physical validation passes.
7. [x] Replace the About destination's placeholder text with the real app description, independent/non-official BookOrbit relationship disclaimer, version/build information, connected server, acknowledgements, and relevant links; user confirmation marks the content complete.
8. [x] Add an audiobook-only Session history menu below the detail actions. Persist play/pause events locally with the book identity, event time, and exact playback timepoint; tapping an event seeks to that point. Keep this data device-only, preserve it across app updates, clear it when the configured server changes, and rely on Android uninstall behavior to remove it with app data.
...
User-confirmed physical validation now covers the remaining reader/media/layout/accessibility/responsive/theme/resume/Preview/offline/edge-state checks and the supported-media matrix, including optional API 33+ external audiobook-control validation. The complete Book Detail `Mark as...` status menu updates server and local/cache state, and tapping an author opens the author screen. Lagrange 1.2.0 is published at GitHub Releases with a signed APK, and release-secret configuration is complete. Release-overlay and Book Detail app-navigation physical validation remain intentionally deferred. Direct OIDC/SSO and optional offline RAR/7z extraction remain deferred.
...
### User-requested work order — 2026-07-26 (active/planned)

The following are new, unchecked, planned items requested by the user. They do not alter any prior historical completed record above.

1. [ ] Add the existing Lagrange app logo centered above the login form inside the login card.
2. [ ] Add BookOrbit per-book metadata-source/provider links to Book Detail. Verified research: BookOrbit Book Detail exposes `providerIds` keyed by metadata provider. Recommendation: a compact `Metadata sources` row directly below Synopsis and above the lower metadata sections, with labeled/icon links opening each provider's URL.
3. [x] Supplement local audiobook Session history with BookOrbit server reading-log data. Verified result: `GET /api/v1/books/{bookId}/sessions` returns analytics sessions with `startedAt`, `endedAt`, `durationSeconds`, `progressDelta`, `endProgress`, `format`, `source`, and `stats`; `GET /api/v1/books/{bookId}/reading-attempts` returns date-level attempts with `outcome`, `totalSessions`, and `totalSeconds`, and neither response carries an exact audio position or file ID. Lagrange now supplements (not replaces) the local Room `AudiobookSessionHistoryStore` — which retains exact play/pause position and seek behavior and remains authoritative for seeking — with a separate audiobook-only Server reading history section on Book Detail, authenticated repository methods/parsers, loading/unsupported/error/empty states, and tests. Server analytics are never treated as an exact audio position.
4. [x] Add a profile-menu `Statistics` destination above Achievements. Verified endpoints: `GET /api/v1/user-statistics/summary`, `daily-reading`, `reading-heatmap`, `reading-source-distribution`, `peak-hours`, `favorite-days`, `session-timeline`, `completion-timeline`, `goal-trajectory`, `progress-funnel`, `completion-latency`, `genre-reading-time`, `reading-pace`, `session-archetypes`, and related `user-statistics` routes. Implemented as a separate lazy-loaded Statistics screen consuming `summary` and `daily-reading`, with graceful unsupported/error/partial-data handling; additional metrics remain future expansion.

...
1. [ ] **Highest priority:** deliver Back 10 / Play-Pause / Forward 30 in the Android pull-down player and lock screen while retaining chapter navigation inside the app player.
   ...
   - [ ] Add focused tests and an OS-facing regression. Query a real session's platform token on API 33+, assert ordered Back-10/Forward-30 `PlaybackState` custom actions/icons, invoke both actions, and verify exact bounded -10/+30 position changes. Retain pre-33 compact/provider coverage and exercise both local Readium and direct streamed Media3 command sets.
   - [ ] Run affected-source-set compilation, focused tests, the full unit/lint/APK gate, and produce a fresh debug APK. Completion requires physical API 33+ pull-down and lock-screen validation for one local and one streamed audiobook, boundaries, paused/background/task-relaunch states, and headset/Bluetooth where available, plus an API 26-32 fallback spot-check.

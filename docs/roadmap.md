...
The current reader/detail, stability, audiobook, cover-aspect, Readium-PDF, tutorial-label, Series-grouping, and profile-menu gate passes 264 JVM tests across 45 suites with zero failures/errors/skips; `testDebugUnitTest`, `lintDebug`, `assembleDebug`, and Android-test APK assembly pass. JVM/compiled coverage includes PDF direct/Preview routing and the generated three-page `Profile.PDF` opening contract, mixed cover ownership/enrichment and persistence, the Room v2 column/default contract, compact audiobook controls/caching/Preview, crash recovery, Browser-first audio relaunch, timing/dismissal, revised chrome/tutorial labels, delayed refresh/download reader ownership, persisted grouping, section ordering, and profile-menu structure. Connected/generated-PDF and target-device PDF UI checks, mixed-library cover geometry, audio relaunch, revised audiobook UI/Preview, and remaining media/device validation remain pending.

...
1. [x] Replace Android system media-player chapter skip actions with 10-second backward and 30-second forward seek actions for audiobook playback, while retaining chapter navigation inside the app player. Automated coverage and the full 295-JVM-test/50-suite gate passes; physical notification, lock-screen, headset, and Bluetooth validation remains pending.
   - [x] Resolve the follow-up perpetual-Preparing regression by returning explicit Media3 dependencies to Readium-compatible 1.4.1 and expressing Replay 10/Forward 30 through the compatible MediaSession custom layout. Service binding is capped at 10 seconds, engine preparation at 30 seconds, and cancellation/error cleanup cannot leave the compact player latched in Preparing. The current 299-JVM-test/50-suite gate, lint, and both APK assemblies pass; connected instrumentation compiles but awaits a device.

...
5. [x] Add only an app-global compact player modeled on the official Audiobookshelf small player: cover, title/author, progress, −15, play/pause, +30, and explicit Close. Normal Read/Preview returns to the browser with playback retained, and the overlay appears across main destinations plus separate Readium EPUB/comic activities. No expanded or fullscreen player exists. Focused tests plus debug and Android-test compilation pass; runtime/device and service/process recreation validation remain.

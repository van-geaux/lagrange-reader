...
1. [ ] Replace the oversized Achievement poster tiles with compact information cards: keep the small server-driven icon in the title row immediately before the title, put lock/unlock state at the row end, and retain description plus progress or earned date below.
2. [ ] Fix the book-detail action row on narrow screens so Read, Preview, Download/Delete local, and More do not hide off-screen without a discoverability cue. Confirm the final responsive treatment before implementation.
...
The completed Achievements step passes 208 JVM tests across 34 suites with zero failures/errors/skips; `testDebugUnitTest`, `lintDebug`, `assembleDebug`, and `assembleDebugAndroidTest` pass. The functional target-server/device flow is validated, but the poster tiles are too large and a compact information-card redesign is now pending. The debug APK is `app/build/outputs/apk/debug/app-debug.apk`.

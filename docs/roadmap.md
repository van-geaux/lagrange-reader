...
The current reader/detail, stability, audiobook, cover-aspect, Readium-PDF, tutorial-label, Series-grouping, and profile-menu gate passes 264 JVM tests across 45 suites with zero failures/errors/skips; `testDebugUnitTest`, `lintDebug`, `assembleDebug`, and Android-test APK assembly pass. Connected coverage proves the generated three-page PDF opens as `Profile.PDF` with exactly three positions; JVM/compiled coverage also includes PDF direct/Preview routing, mixed cover ownership/enrichment and persistence, the Room v2 column/default contract, compact audiobook controls/caching/Preview, crash recovery, Browser-first audio relaunch, timing/dismissal, revised chrome/tutorial labels, delayed refresh/download reader ownership, persisted grouping, section ordering, and profile-menu structure. Target-device PDF UI checks, mixed-library cover geometry, audio relaunch, revised audiobook UI/Preview, and remaining media/device validation remain pending.

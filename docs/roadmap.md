...
21. [x] Restyle Achievements as Library-like adaptive poster tiles with a 128 dp minimum width. Map the server-provided `iconName` across the official BookOrbit vocabulary to semantic Android Material equivalents, render the symbol prominently with a small locked/unlocked indicator, and move the name, two-line description, category/rarity, conditional locked progress, and earned date below the poster. Retain the summary and Unlocked then Locked sections. Physical visual and responsive validation remains pending.
...
The completed Achievements step passes 208 JVM tests across 34 suites with zero failures/errors/skips; `testDebugUnitTest`, `lintDebug`, `assembleDebug`, and `assembleDebugAndroidTest` pass. The functional target-server flow is validated. Poster-tile coverage maps representative official icon names and the safe fallback at JVM level; compiled instrumented UI fixtures use official icon names and assert two symbol nodes plus locked progress. Physical visual, responsive, and edge-state validation remains pending. The debug APK is `app/build/outputs/apk/debug/app-debug.apk`.

...
Version 0.2.7 includes the July 20 reader/detail follow-ups. Target-device feedback confirms the reader remains open during sync/refresh/download activity, Exit works, and the tutorial timing/dismissal and revised label readability behave correctly. Series details persist Library/Format/none grouping globally; target-device feedback confirms cross-library separation, persistence, and the inline section label/divider layout. The profile dropdown now places Achievements, Options, and About above a divider before session actions, and its ordering is device-validated. The full gate passes 262 JVM tests across 44 suites plus lint and both APK assemblies; physical checks of mixed-library cover geometry, the Options cog icon, divider semantics, About routing, accessibility, and large text remain.
...
The current reader/detail, stability, audiobook, cover-aspect, tutorial-label, Series-grouping, and profile-menu gate passes 262 JVM tests across 44 suites with zero failures/errors/skips; `testDebugUnitTest`, `lintDebug`, `assembleDebug`, and Android-test APK assembly pass. JVM/compiled coverage includes mixed cover ownership/enrichment and persistence, the Room v2 column/default contract, compact audiobook controls/caching/Preview, crash recovery, Browser-first audio relaunch, timing/dismissal, revised chrome/tutorial labels, delayed refresh/download reader ownership, persisted grouping, section ordering, and profile-menu structure. Mixed-library cover geometry, audio relaunch, revised audiobook UI/Preview, and remaining media/device validation remain pending.

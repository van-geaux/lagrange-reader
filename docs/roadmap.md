...
1. [x] Replace Android system media-player chapter skip actions with 10-second backward and 30-second forward seek actions for audiobook playback, while retaining chapter navigation inside the app player. Automated coverage and the full 295-JVM-test/50-suite gate passes; physical notification, lock-screen, headset, and Bluetooth validation remains pending.
   - [x] Resolve the follow-up perpetual-Preparing regression by returning explicit Media3 dependencies to Readium-compatible 1.4.1. Physical feedback showed that 1.4.1 ignores player-command custom-layout buttons in its default notification, so register a provider that emits compact Back 10 / Play-Pause / Forward 30 and filter previous/next from external player commands. Explicitly configure local Readium from its 15-second rewind default to the same 10/30-second increments as direct streaming. The current 300-JVM-test/50-suite gate, lint, and both APK assemblies pass; connected instrumentation compiles but awaits a device.

...
The July 19 EPUB compatibility work order below overrides the general phase sequence. The current staged proof routes only EPUB Preview through Readium Kotlin Toolkit 3.0.0; normal Read remains on the custom reader. Samsung Galaxy S24 Preview verification must pass before deciding whether to extend Readium or resume later book-detail and reader-chrome work.
...
All samples correctly identify a JPEG manifest item with EPUB3 `properties=cover-image` (Zero Damage also carries legacy metadata), so missing OPF cover metadata is not the cause. The custom reader retains its narrow `normalizeBitmapOnlySvgPages` fallback. The new staged proof pins Readium Kotlin Toolkit 3.0.0 to the current Kotlin 1.9.24/compileSdk 35 toolchain and routes only EPUB Preview into `ReadiumEpubPreviewActivity`; normal EPUB Read, comics, and audiobooks keep their existing implementations.

Permanent coverage now includes `ReadiumEpubPreviewRoutingTest`, which confines the spike to EPUB Preview, and `ReadiumEpubOpenInstrumentedTest`, which generates a bitmap-only SVG-cover EPUB and verifies opening plus `publication.cover()` and `coverFitting()`. The full gate passes 216 JVM tests across 36 suites with zero failures/errors/skips, `lintDebug`, `assembleDebug`, and `assembleDebugAndroidTest`; the focused Readium instrumentation test passes. The four supplied samples opened in the Readium activity on the emulator, and screenshots visually confirmed each cover. Samsung physical success remains unconfirmed.

Execute the current work in this order:

1. Install `app/build/outputs/apk/debug/app-debug.apk` on the Samsung Galaxy S24 and Preview `your name.`, Overlord Vol. 1, Overlord Vol. 5, and `Zero Damage Sword Saint Volume 4`. Confirm covers and later inline images render, Back returns cleanly, and Preview leaves normal progress untouched. Normal Read intentionally remains the custom fallback. Decide whether to widen Readium only after this phone proof passes; confirm comic and audiobook behavior remains unchanged.

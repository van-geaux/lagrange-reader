...
- Finish cache limits/eviction, non-range fallback behavior, interrupted-request recovery, offline transitions, comic implementation, and representative large-file device validation. Audio has deterministic JVM header/authentication coverage and a compiled Android bounded-range regression; connected execution remains pending because no device was attached during this step.
...
The current reader/detail, stability, audiobook, cover-aspect/alignment, Readium-PDF, tutorial-label, Series-grouping, profile-menu, orientation, fixed Library controls, and audio-range gate passes 271 JVM tests across 47 suites with zero failures/errors/skips; `testDebugUnitTest`, `lintDebug`, `assembleDebug`, and Android-test APK assembly pass. Connected coverage proves the generated three-page PDF opens as `Profile.PDF` with exactly three positions; JVM/compiled coverage also includes authenticated Readium header policy and a bounded remote audio-range request, poster/shelf cover-slot alignment, PDF direct/Preview routing, mixed cover ownership/enrichment and persistence, Room v2, compact audiobook controls, crash recovery, Browser-first audio relaunch, revised reader interaction, grouping, and profile-menu structure. Target-device remote-audio, PDF UI, mixed-library cover alignment, audio relaunch, revised audiobook UI/Preview, and remaining media/device validation remain pending.

...
Version 0.2.7 completes the shared EPUB/comic tap-zone tutorial. Samsung Galaxy S24 validation remains open; the next work is target-device/accessibility/responsive/theme/timing/resume/Preview/offline validation and remaining edge/media coverage.
...
4. Completed in version 0.2.7: after navigator readiness, every EPUB/comic Read and Preview entry shows an above-all-UI, tap-consuming tutorial. Three equal full-height thirds are Previous RGB(255,114,118), Menu RGB(0,0,0), and Next RGB(144,238,144), each alpha 0.5. The exact 1,000 ms dismissal starts on first pre-draw. Version 0.2.6 lightweight chrome remains unchanged.
...
The tutorial step passes 241 JVM tests across 41 suites with zero failures/errors/skips; `testDebugUnitTest`, `lintDebug`, `assembleDebug`, and `assembleDebugAndroidTest` pass. Three JVM contract cases and a third compiled Compose chrome/tutorial case cover timing, colors, geometry, and labels. Five focused connected tests pass and verify tutorial launch/dismissal before center-tap chrome. API 37 generic Compose execution retains the Espresso limitation.

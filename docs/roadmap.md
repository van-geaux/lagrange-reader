...
The July 19 EPUB compatibility work order below overrides the general phase sequence: no book-detail, reader-chrome, or remaining validation work advances until the virtual no-store document candidate passes the Samsung Galaxy S24 EPUB retest.
...
The extraction-root and appassets-root/SVG candidates remained insufficient on the Samsung Galaxy S24. The user supplied three valid, unencrypted sample EPUBs in `sample/`: `01 - your name.epub`, `04 - Zero Damage Sword Saint_ I Enrolled in a Magic School and Wound Up in a Contract with the Demon Queen_ Volume 4.epub`, and `05 - Overlord, Vol. 5.epub`. Calibre renders all covers/images. Exact-sample diagnostics under Android WebView 149 also decoded and laid out every image-bearing spine chapter through the prior path, proving the EPUBs/JPEGs valid and showing a device-path difference without by itself identifying the S24 cause.

The new production implementation replaces `loadDataWithBaseURL` data-document navigation in both visible and hidden WebViews with `EpubWebViewAssetSession`. Styled chapter bytes are served as unique no-store virtual documents at `https://appassets.androidplatform.net/_bookorbit-reader/<uuid>.html` and opened with `loadUrl`. An escaped injected `<base>` is inserted before EPUB stylesheet links and preserves the original chapter directory for relative, parent-relative, and root-relative resources. The current book remains mounted at appassets `/`; both WebViews use `LOAD_NO_CACHE`; broad file/content access remains disabled; and a session retains at most four virtual document byte arrays.

Exact-sample diagnostics reran every image-bearing spine chapter from all three supplied EPUBs through the production path, with every referenced `img`/SVG image decoded and given nonzero layout. Permanent plain-WebView `EpubAssetLoadingInstrumentedTest` passes 1/1 on emulator and proves the main document plus parent/root/SVG images remain on appassets and lay out inside a sample-like 95%-height wrapper. The obsolete Compose `loadData` image test is removed. The full gate passes 215 JVM tests across 35 suites with zero failures/errors/skips, `lintDebug`, `assembleDebug`, and `assembleDebugAndroidTest`. Samsung physical success remains unconfirmed.

Execute the current work in this order:

1. Install `app/build/outputs/apk/debug/app-debug.apk` on the Samsung Galaxy S24 and retest all three supplied EPUBs plus a mixed cover-missing/later-images-working title in normal Read, nonlocal Preview after relogin, and local download where applicable. Do not claim resolution or advance other work until every cover/illustration renders and CBZ/CBR remains unaffected.
...
The original book-detail reading-status overflow step passes 183 JVM tests across 30 suites with zero failures/errors/skips; `testDebugUnitTest`, `lintDebug`, `assembleDebug`, and `assembleDebugAndroidTest` pass. The latest compact UI follow-up removes that overflow and keeps the same context-sensitive status logic in a directly labeled wrapping tile.

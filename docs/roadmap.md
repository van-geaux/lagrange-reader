...
Version 0.2.7 includes the July 20 reader/detail follow-ups. Target-device feedback confirms the reader remains open during sync/refresh/download activity, Exit works, and the tutorial timing/dismissal and revised label readability behave correctly. Series details persist Library/Format/none grouping globally; target-device feedback confirms cross-library separation, persistence, and the inline section label/divider layout. The profile dropdown now places Achievements, Options, and About above a divider before session actions, and its ordering is device-validated. The full gate passes 252 JVM tests across 42 suites plus lint and both APK assemblies; physical checks of the Options cog icon, divider semantics, About routing, accessibility, and large text remain.
...
4. [x] Establish persistent audiobook playback with Readium 3.0.2's media `AudioNavigator` and ExoPlayer adapter hosted by a foreground Media3 `MediaSessionService`. Media-playback/notification permissions, an application-scoped controller, local M4B and other recognized audio opening, and an explicit background/task-removal survival policy are in place; focused format tests and compilation pass. Finish device validation plus notification/control, audio-focus/noisy, progress/offline/Preview, service-restoration, and explicit-close validation during UI wiring.
5. [ ] Add only an app-global compact player modeled on the official Audiobookshelf small player: cover, title/author, progress, play/pause, timed rewind/forward, and explicit Close above every destination. Do not add an expanded or fullscreen audiobook player.

...
- [ ] Replace Achievement posters with compact information cards: small server-driven symbol immediately before the title, lock/unlock state at the title row's end, and description plus progress/date below.
- [ ] Make every book-detail action discoverable on narrow screens. The current horizontally scrolling Read/Preview/Download-or-Delete/More row can hide trailing actions without a cue; confirm whether to wrap, condense, or add an explicit overflow treatment before implementation.

...
Automated gate: 264 JVM tests across 45 suites, lint, debug APK assembly, and Android-test APK assembly pass. Tutorial readability, Series grouping/persistence, section label/divider layout, profile-menu ordering, and hardened audiobook opening/playback are target-device validated; connected/generated-PDF and target-device PDF checks, mixed-library cover geometry, audio relaunch, authenticated Preview, the revised audio layout, icon/divider/About, and accessibility checks remain open.
...
- [ ] Execute the generated three-page PDF test on a connected device and validate normal/Preview, exact resume/progress, invalid-file failure, page controls/chrome/tutorial, orientation, system bars, compact-audio coexistence, accessibility, large text, and responsive layouts on the target device.
...
- Readium PDF uses the same shared page-reader interaction contract through Pdfium; run the compiled generated-PDF opening test and validate the complete UI/progress/Preview behavior on device before marking it device-validated.

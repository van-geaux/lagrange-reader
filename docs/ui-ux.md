...
Audited implementation status: code and automated verification are complete; genre query compatibility and scope are target-server validated. Local books exposes active/failed transfer status conditionally, persists interrupted attempts separately from completed local copies across force-close/reopen, keeps failed first downloads remote-only, preserves failed updates' old local copy, and exposes `Update local` plus `Delete local` for that update-failure state. Local books fills the available list area when no transfer section is present. Physical-device download-lifecycle validation remains pending. Direct OIDC/SSO remains an open backlog item.
...
Latest target-device validation confirms the latest reader/library/media groups, including orientation persistence, fixed Library controls, Authors jump rail, destination icons, exact reading-state shelves, remote media, and the remaining reader/device matrix. Local batch deletion and the new Delete local menus are automated-verified, but physical-device validation remains pending because no adb target was available. Splash/loading is accepted because the app opens directly to content quickly; it is no longer blocking. The Book Detail Other versions row is target-device validated, including responsive, accessibility, and large-text behavior. The haptic setting and all explicit app haptics were removed by user direction.

Latest implementation: Delete local immediately returns the still-open detail action to Download and refreshes Local books; incomplete Local books summaries recover thumbnails and related metadata from the latest cached rich detail; and Options now dismisses retained book-detail state before opening. Delete local is automated-verified, with physical-device validation pending; the other browser/local flows and server-wide Home/Currently Reading aggregation are target-device validated. Comic routing supports online CBZ/CBR/CB7 plus offline ZIP/CBZ and client-side RAR4/RAR5/7z normalization into a cached CBZ, while recording page progress. Fullscreen interactions, online/local comic reading, and user-confirmed offline downloaded CBR/CB7 opening work; encrypted archives remain intentionally unsupported. Direct OIDC/SSO remains deferred.
...
- Interface implementation complete: lock current orientation toggle; one flat app Theme list (Follow system, Light, Charcoal, Warm black, OLED black) applied immediately; default opening screen (Home, Library, Local books) on fresh start; Reduce motion/animations using immediate catalog jumps. Follow system uses Light or Charcoal according to Android, explicit choices remain fixed, and legacy Dark migrates to Charcoal. Lock orientation now persists the portrait/landscape orientation active when enabled and restores it explicitly in the main app and separate Readium activities; legacy enabled values default to portrait rather than inheriting another app's landscape orientation. The full gate passes 267 JVM tests across 46 suites, lint, and both APK assemblies, while the new cross-app/cold-start target-device regression remains pending. Default opening screen, Reduce motion, and all five themes are device-validated. The former haptic setting and explicit app haptics are removed.
- Library card-size implementation and physical validation complete: Options exposes one global persisted choice across all libraries and card types—Small (88 dp grid / 84 dp shelf), Medium (110 dp / 105 dp), or Large (132 dp / 126 dp). Focused tests, compilation, lint, and APK assembly pass; the user confirmed the setting works correctly on a physical device.
...
Current refinement: book poster cards use roughly half of the first candidate size, search uses BookOrbit's global query endpoint, and covers load through the authenticated API client. Series shelf cards open an ordered series detail list. Book selections open a detail screen with Read/Continue, Download, and local-copy actions instead of launching content immediately. The browser shell now uses Home, Libraries, and More in a bottom bar; More expands to Series, Authors, and Local books, while the profile menu exposes Options, Achievements, and About, then Change server immediately above Log out/Sign in. The Home top bar carries the launcher mark, while Library uses the selected library name as its selector and separates Recommended shelves from complete Room-cached Browse content.

Detail refinement: book details mirror the reader-relevant BookOrbit metadata and use one 46 dp-high action row. Read/Preview stay labeled; a nonlocal icon slot maps Download/Retry/Cancel and remains disabled in place when unavailable/offline; for downloaded books, `Delete local` is available in the More/context menu across Book Detail and book-card surfaces, absent for nonlocal books, and reuses the existing confirmation behavior. Local Update local or Cancel update also live in More. The reading-status action is labeled `Mark as...` and opens the complete ordered menu: Unread, Want to read, Reading, Rereading, On hold, Abandoned, Read, and Skimmed. More appears whenever anything is hidden, and only Read/Preview become weighted at extreme widths. Audiobook Session history is not inline in Book Detail: a logo-only button appears before Play only for currently-reading audiobooks, and opens a scrollable overlay inside Book Detail with the book title, Play/Pause labels, event timestamps, exact positions, Clear, and a close button. The series eyebrow/neighbor controls, independently wrapped Genre/Tag chips, compact metadata cards, and full-screen dismissible cover viewer remain unchanged.

### Download lifecycle refinement — 2026-07-28
...
- [x] Add the completed About destination through the profile dropdown. It presents the real Lagrange Reader app description, explains the independent BookOrbit relationship and non-official disclaimer, shows installed version/build information and the connected server, and includes acknowledgements plus links to Lagrange Reader, BookOrbit, Readium, and the license/notices.
...
- Delete local, whenever applicable, is always in the three-dot/More overlay and never inline across Book Detail and book-card surfaces. It appears only for downloaded books and reuses the existing confirmation behavior.
...
- [x] Validate the final layout and behavior across EPUB/comic Read and Preview, status-bar insets, large text, portrait/landscape, themes, refresh/sync activity, and repeated opens. User-confirmed physical validation is complete.
...
Automated gate: 265 JVM tests across 46 suites, lint, debug APK assembly, and Android-test APK assembly pass. The generated three-page PDF connected test also passes on Medium_Phone AVD API 17. Tutorial readability, Series grouping/persistence, section label/divider layout, profile-menu ordering, and hardened audiobook opening/playback are target-device validated; physical PDF checks, mixed-library cover alignment, audio relaunch, authenticated Preview, the revised audio layout, icon/divider/About, and accessibility checks remain open.
...
- [x] Audit and encode one explicit `ReadiumPublicationRoute` matrix: EPUB/KEPUB direct EPUB, PDF direct Readium PDF, supported audio direct, CBZ direct image, CBR/CB7 cached-CBZ normalization, and MOBI/AZW/AZW3/FB2 `UNSUPPORTED_EBOOK`. Unsupported ebooks remain `MediaKind.UNKNOWN` and show `This file format is not supported.` instead of falsely opening as EPUB. Focused helper/update tests pass; conversion is deferred outside current product scope.
- [x] Migrate PDF from legacy Compose `PdfRenderer` to `readium-adapter-pdfium:3.0.2`, parsing with `PdfiumDocumentFactory`/`PublicationOpener` and navigating with `PdfNavigatorFragment`/`PdfiumEngineProvider` in `ReadiumPdfReaderActivity`. Preserve exact normal locator resume/progress, Preview page-1 isolation, 25% edge turns, center chrome, page rail/footer/options/tutorial, orientation/keep-awake/system bars, global compact audio, and explicit invalid-PDF failure. Two JVM routing tests pass, and the connected generated three-page test passes on Medium_Phone AVD API 17 as `Profile.PDF` with exactly three positions. Physical target-device UI validation remains pending. EPUB now always uses Readium, while non-normalizable comics show conversion/reconnect guidance instead of opening the legacy comic reader.
- [x] Respect each owning library's exact `coverAspectRatio`: `2/3` remains portrait, `1/1` is genuinely square, and missing/unknown values fall back to portrait. Persist the contract through Browser/active-reader/detail/Room caches and resolve mixed results by owner. Shared/fullscreen/compact-player covers use true aspect geometry with Crop and no artificial padding. Poster/shelf cards reserve a portrait-height `BookCardCoverSlot`, bottom-center the actual cover so bottoms and labels align, and attach More to the image. `BookCoverLayoutTest` guards the contract; physical mixed-library alignment validation remains pending.
- [x] Build persistent Readium audio with an application-scoped controller. It samples progress every 1.5 seconds independently of UI/background navigation; Preview is isolated from normal progress and session history, explicit Close publishes final normal progress and stops the service, Readium handles focus/noisy output, and Android 13 notification permission is requested on first playback.
...
- [x] Validate relaunch on the user device: play audio, leave or close the app without closing the player, reopen, and confirm Browser plus compact player appears without Preparing. Also validate authenticated Preview and the revised compact layout, then notification/lock-screen/headset/Bluetooth controls, offline/interruption behavior, accessibility/responsive layout, and service/process recreation. User-confirmed physical validation is complete.
- [x] Validate PDF normal/Preview, exact resume/progress, invalid-file failure, page controls/chrome/tutorial, orientation, system bars, compact-audio coexistence, accessibility, large text, and responsive layouts on the physical target device. User-confirmed physical validation is complete.
- [x] Validate square/portrait and mixed-library layouts plus background audio across process/service recreation, interruptions, offline/online transitions, progress sync, Preview isolation, accessibility, large text, orientation, themes, and every supported format. MOBI/AZW/AZW3/FB2 remain intentionally unsupported. User-confirmed physical validation is complete.
- [x] Keep EPUB/PDF whole-file preparation, but replace full-file online preparation for M4B/MP4/audiobooks and CBZ/CBR/CB7 with authenticated range/lazy-page streaming plus bounded disposable caches. The user selected the Readium-compatible remote-resource architecture, retaining the existing service/progress model. Implementation and physical validation are complete.
- [x] Keep the Library Browse statistics and filter/collapse or selection toolbar outside the lazy catalog grid so it remains fixed during scrolling and pull-to-refresh gestures. Collapse-anchor restoration and jump-rail targets use direct grid indexes after removal of the former scrolling header item; compiled Compose coverage verifies stable toolbar bounds after scrolling to the final card. The full gate passes 267 JVM tests across 46 suites, lint, and both APK assemblies.
- [x] Add `Delete local` to the Local books multi-selection toolbar only. It follows the saved confirmation preference, keeps the selection when Cancel is chosen, clears it after confirmation, removes every successful local copy in one refresh, and leaves failed copies visible with a partial-result message. Focused JVM and compiled Compose coverage pass; target-device validation remains.
- [x] Add the shared #/A-Z rail to Authors after loading the complete name-sorted catalog. Only represented labels are active, unavailable labels retain disabled semantics, Reduce motion jumps immediately, exact targets account for the statistics header, and the card gutter clears the rail. Compiled coverage verifies paging, semantics, no Load more, and responsive separation; target-device validation remains.
- [x] Apply the selected filled semantic destination icons: bookshelf for Libraries, bookmarked collection for Series, group for Authors, and offline download for Local books. Each has distinct accessibility semantics and compiled UI coverage; theme/scale validation remains for the target device.

### Checkpoint 5: Other media readers - Readium PDF/comics implemented, device validation pending
...
- Readium PDF uses the same shared page-reader interaction contract through Pdfium. Its generated three-page connected opening test passes; validate the complete UI/progress/Preview behavior on the physical target device before marking it device-validated.
...
Cache smoothing keeps fitted books fully decoded when possible and otherwise evicts least-recently-used pages. Synchronous hits prevent placeholder flashes, known aspect ratios hold page height during reload, and visible/±2-prefetch loads deduplicate. Memory pressure trims/clears the cache and close clears the book-scoped state. Continuous Swipe up/Menu/Swipe down tutorial behavior remains. Physical long-book down/up smoothness and memory stability still require validation.
...
User-confirmed physical validation covers the remaining reader/media/layout/accessibility/responsive/theme/resume/Preview/offline/edge-state checks and supported-media matrix. Book Detail's complete `Mark as...` menu updates BookOrbit and local/cache state, and tapping an author opens the author screen. Book Detail app-navigation physical validation remains intentionally deferred. The historical `v1.1.0` GitHub APK upload is complete; release-secret configuration remains pending. Direct OIDC/SSO and optional offline RAR/7z extraction remain deferred.
...
A compact `Metadata sources` row sits directly below Synopsis and above the lower metadata sections (Publication, Identifiers, Library/file). It renders one labeled link per BookOrbit metadata `providerIds` entry, each opening that provider's URL.

The Android UI now links all 14 of BookOrbit's current provider keys: `google`, `goodreads`, `amazon`, `hardcover`, `openLibrary`, `itunes`, `audible`, `librofm`, `kobo`, `lubimyczytac`, `ranobedb` (RanobeDB, opening `https://ranobedb.org/book/{bookId}`), `audnexus` (Audnexus, opening `https://api.audnex.us/books/{bookId}`), `comicvine` (ComicVine, opening `https://comicvine.gamespot.com/issue/{bookId}/`), and `aladin` (Aladin, opening `https://www.aladin.co.kr/shop/wproduct.aspx?ItemId={bookId}`). Audnexus opens a provider API endpoint rather than a human-facing page.

Provider labels and URL builders are centralized in `app/src/main/java/com/vangeaux/lagrange/ProviderLinks.kt`, so adding a verified provider requires one registry entry plus a focused test.

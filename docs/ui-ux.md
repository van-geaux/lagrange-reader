...
- [ ] Playback continues when the user presses Back, navigates elsewhere in Lagrange, locks the phone, or switches apps. Android exposes an ongoing media player in the notification shade and lock screen with artwork, metadata, progress, play/pause, and timed rewind/forward; headset/Bluetooth controls, audio focus, and unplug behavior follow platform expectations. Explicit Close is the stop/end-session action.
- [ ] Keep a player above all in-app destinations until explicitly closed. Closely follow the official Audiobookshelf app: compact cover/title/author/progress with play/pause and timed jumps; tap or gesture expands to a cover-colored/gradient full player with collapse, More/Close, cover, title/author, elapsed/remaining seek track, chapter/timed navigation, speed, sleep timer, and chapters. Bookmarks and casting are later parity only when supported.

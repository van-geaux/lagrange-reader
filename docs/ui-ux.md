...
Continuous comic mode uses vertical Swipe up/Menu/Swipe down tutorial regions and prefetches the two previous and two next pages; paginated mode retains LR/RL Previous/Menu/Next. The full gate now passes 292 JVM tests across 50 suites with zero failures/errors/skips, lint, and both APK assemblies. Validate tutorial labels/layout, long-book scroll-down/up smoothness, and adaptive cache memory stability on device.

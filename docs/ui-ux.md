...
The functional prototype is stable enough for UI/UX work to continue, but all planned UI work remains behind EPUB image compatibility. Commit `4854b0b` still failed on the Samsung Galaxy S24. Sample comparison isolates the failing `your name.` and Overlord opening covers as bitmap-only SVG pages, while Overlord Vol. 1's later ordinary image and no-SVG `Zero Damage Sword Saint Volume 4` work. The new pre-pagination normalization converts only one-raster-image/no-vector-primitives SVGs to fitted ordinary images; genuine vector SVGs and ordinary images remain unchanged. Automation and the targeted emulator test pass, but the S24 retest must succeed before the regression is considered fixed. Comic/audiobook work remains later.
...
- Commit `4854b0b` retained the S24 failure. `normalizeBitmapOnlySvgPages` now replaces only single-raster/no-vector-primitives SVG covers with eager/sync ordinary images and strong `bookorbit-svg-raster` fitting CSS, preserving source/id/class/accessibility text. The permanent emulator test proves the replacement decodes/fits and that a genuine vector SVG containing a `rect` remains SVG and lays out. The focused test passes 1/1 and the full gate remains 215 JVM tests across 35 suites plus lint and both APK assemblies. S24 confirmation remains required.
...
1. [ ] Install the new APK on the Samsung Galaxy S24 and retest `your name.`, Overlord Vol. 1, Overlord Vol. 5, and the no-SVG `Zero Damage Sword Saint Volume 4` control. Confirm bitmap-only SVG covers render/fill correctly, Overlord Vol. 1 chapter 3 and other ordinary/genuine-vector images remain unchanged, and CBZ/CBR stays unaffected. Keep every later item blocked until this passes.

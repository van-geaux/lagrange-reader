...
Detail refinement: book details mirror the reader-relevant BookOrbit metadata and use one 46 dp-high action row. Read/Preview stay labeled; a nonlocal icon slot maps Download/Retry/Cancel and remains disabled in place when unavailable/offline; local Delete and Update/Cancel update live in More. The reading-status action is labeled `Mark as...` and opens the complete ordered menu: Unread, Want to read, Reading, Rereading, On hold, Abandoned, Read, and Skimmed. More appears whenever anything is hidden, and only Read/Preview become weighted at extreme widths. Audiobook Session history is not inline in Book Detail: a logo-only button appears before Play only for currently-reading audiobooks, and opens a scrollable overlay inside Book Detail with the book title, Play/Pause labels, event timestamps, exact positions, Clear, and a close button. The series eyebrow/neighbor controls, independently wrapped Genre/Tag chips, compact metadata cards, and full-screen dismissible cover viewer remain unchanged.

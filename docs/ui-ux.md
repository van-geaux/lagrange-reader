...
- [x] Add the approved dedicated Previous/Next row directly below book identity/series metadata. It now uses transparent borderless 46 dp controls matching detail actions and retains the first complete series load across adjacent navigation. Basic navigation is target-device validated; physical validation of the new visuals plus responsive, long-title, loading, and offline states remains pending.
- [x] Replace the old app Dark option with one flat ordered list: Follow system, Light, Charcoal, Warm black, OLED black. All themes work on the target device; detailed contrast/system-bar edge validation remains pending.
...
- [x] Add a functional profile Achievements destination and redesign it as a Library-like adaptive poster grid with a 128 dp minimum tile width. Keep the earned/available summary and Unlocked then Locked sections; map official server `iconName` values to prominent semantic Android Material symbols, retain a small lock-state indicator, and place the name, two-line description, category/rarity, conditional locked progress, and earned date below each poster. Functional target-server validation passed; physical visual and responsive validation remains pending.

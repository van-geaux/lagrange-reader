...
- [ ] Validate PDF normal/Preview, exact resume/progress, invalid-file failure, page controls/chrome/tutorial, orientation, system bars, compact-audio coexistence, accessibility, large text, and responsive layouts on the physical target device.
- [ ] Validate square/portrait and mixed-library layouts plus background audio across process/service recreation, interruptions, offline/online transitions, progress sync, Preview isolation, accessibility, large text, orientation, themes, and every supported format. MOBI/AZW/AZW3/FB2 remain intentionally unsupported.
- [ ] Keep EPUB/PDF whole-file preparation, but replace full-file online preparation for M4B/MP4/audiobooks and CBZ/CBR/CB7 with authenticated range/lazy-page streaming plus bounded disposable caches. The user selected the Readium-compatible remote-resource architecture, retaining the existing service/progress model.

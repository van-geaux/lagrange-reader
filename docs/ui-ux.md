...
The functional prototype is stable enough for UI/UX work to continue, but all planned UI work remains behind EPUB image compatibility. The extraction-root build still failed physically: `your name.` and `Overlord, Vol. 5` show no images, some books miss the cover but show later images, and `Zero Damage Sword Saing Vol. 4` shows all images as the control. The new candidate mounts each extracted book at appassets `/` for root-relative resources and preserves full-page SVG wrapper dimensions while sizing normal images separately. Automation passes, but the user must retest the affected/mixed/control samples before the regression is considered fixed. The manga unsupported-format regression remains fixed; CBZ/CBR images work on the target device. CB7 and remaining downloaded-comic validation remain open, offline CBR/CB7 extraction is optional, and audiobook validation remains deferred without a sample.
...
- The extraction-root-only EPUB fix passed automation but failed physically on multiple formats. The new appassets-root/SVG candidate passes 214 JVM tests across 35 suites with zero failures/errors/skips, lint, debug APK assembly, and Android-test APK assembly. The regression chapter combines parent-relative and root-relative `img` references with a full-page SVG `xlink:href`, asserts both images load and SVG height is nonzero, and a JVM pagination test prevents SVG `height:auto`. Per-book root scoping, canonical escape rejection, and disabled broad file/content access remain unchanged. Physical target-device confirmation remains required.
...
1. [ ] Physically verify the appassets-root/SVG candidate using `your name.`, `Overlord, Vol. 5`, a mixed cover-missing/later-images-working title, and the fully working `Zero Damage Sword Saing Vol. 4` control. Cover parent-relative, root-relative, and full-page SVG resources in opened/local and nonlocal Preview paths; confirm CBZ/CBR remains unaffected. Keep every later item blocked until this passes.

...
- [x] Build the persistent playback foundation with Readium 3.0.2's media `AudioNavigator`, ExoPlayer adapter, foreground Media3 session service, playback/notification permissions, application-scoped controller, recognized local-audio opening, and an explicit background/task-removal survival policy. Focused format tests and compilation pass; device validation remains pending.
  - [ ] Wire app screens and validate notification/lock-screen/headset/Bluetooth controls, audio focus/unplug behavior, progress/offline/Preview handling, restoration, and explicit Close as the stop/end-session action.
- [ ] Keep one compact player above all in-app destinations until explicitly closed. Follow only the official Audiobookshelf small player as closely as appropriate: compact cover, title/author, progress, play/pause, timed rewind/forward, and Close. Do not add tap-to-expand, a full player, or a fullscreen audiobook surface.

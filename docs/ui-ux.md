...
Automated gate: 258 JVM tests across 44 suites, lint, debug APK assembly, and Android-test APK assembly pass. Tutorial readability, Series grouping/persistence, section label/divider layout, profile-menu ordering, and the earlier audio crash/open fix are target-device validated; the July 21 audio preparation hardening, revised audio UI/Preview, icon/divider/About, and accessibility checks remain open on device.
...
- [ ] Revalidate audiobook Play and Preview on the user device with the fresh APK, including interrupting Preview during preparation and retrying it. Then validate the revised compact UI plus notification/lock-screen/headset/Bluetooth controls, offline/interruption behavior, accessibility/responsive layout, and service/process recreation.

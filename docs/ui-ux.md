...
Automated gate: 264 JVM tests across 45 suites, lint, debug APK assembly, and Android-test APK assembly pass. The generated three-page PDF connected test also passes on Medium_Phone AVD API 17. Tutorial readability, Series grouping/persistence, section label/divider layout, profile-menu ordering, and hardened audiobook opening/playback are target-device validated; physical PDF checks, mixed-library cover geometry, audio relaunch, authenticated Preview, the revised audio layout, icon/divider/About, and accessibility checks remain open.
...
- [x] Audit and encode one explicit `ReadiumPublicationRoute` matrix: EPUB/KEPUB direct EPUB, PDF direct Readium PDF, supported audio direct, CBZ direct image, CBR/CB7 cached-CBZ normalization, and MOBI/AZW/AZW3/FB2 EPUB normalization. Normalization-required ebooks remain `MediaKind.UNKNOWN` instead of falsely opening as EPUB. Focused helper/update tests pass; PDF execution is implemented while ebook conversion remains pending and must not be presented as supported yet.
...
- [x] Respect each owning library's exact `coverAspectRatio`: `2/3` remains portrait, `1/1` is genuinely square, and missing/unknown values fall back to portrait. Persist the contract through Browser/active-reader/detail/Room caches and resolve mixed search/author/series/local results by owner. Shared book/fullscreen and compact-player covers use true aspect geometry with Crop and no artificial vertical padding. Automated parsing, ownership, persistence, Room-column/default, and rendering-contract coverage passes; physical mixed-library square/portrait validation remains pending.
...
- [ ] Validate square/portrait and mixed-library layouts plus background audio across process/service recreation, interruptions, offline/online transitions, progress sync, Preview isolation, accessibility, large text, orientation, themes, and every migrated format. MOBI/AZW/AZW3/FB2 conversion remains pending.

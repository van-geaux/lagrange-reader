...
- More places the `Annotations` destination exactly between `Authors` and `Local books`. The global screen aggregates annotations across accessible books/libraries, supports annotation/note search and active/trash, note, sort, and pagination filters, and shows loading, retry, and empty states. Tapping a positioned EPUB item opens its CFI/text quote in an isolated Preview launch, renders only the tapped highlight/underline with its saved color/style, and does not alter the book's normal reading progress, locator, session, or active-reader state; positioned PDF items open their saved page and unpositioned items remain informational.
- Each active/trash annotation card exposes a three-dots overflow menu; long-pressing the card opens the identical menu, while a normal tap still opens the annotation's Preview. Active-card actions are edit note, change highlight color/style, copy selected text, and move to trash; trash-card actions are restore and permanently delete. Move-to-trash and permanent-delete both require a confirmation dialog before proceeding. Edit note, color/style change, and move-to-trash use the durable offline mutation queue; restore and permanent delete are authenticated online operations, and their errors are surfaced to the user rather than queued.
- The annotation menu starts with `Open book details`, which uses the same browser detail state as selecting a book from Home, followed by `Go to annotation`, which uses the same isolated Preview navigation as tapping the annotation card. These entries appear before the existing edit/style/copy/trash actions.
- EPUB text selection retains the native Android horizontal `ActionMode` for Copy, Share, Web search, Highlight, and Highlight + Note. The app does not replace it with a custom vertical menu, preserving standard selection behavior and the existing selection-capture flow.

...
- Interface implementation complete: lock current orientation toggle; preference-aware perceptible haptic feedback for existing request sites; one flat app Theme list (Follow system, Light, Charcoal, Warm black, OLED black) applied immediately; default opening screen (Home, Library, Local books) on fresh start; Reduce motion/animations using immediate catalog jumps. Follow system uses Light or Charcoal according to Android, explicit choices remain fixed, and legacy Dark migrates to Charcoal. Orientation locking, default opening screen, and Reduce motion are device-validated. Haptic perceptibility and five-theme device validation remain open; expanding haptic coverage is a separate design decision.
...
- [x] Add the approved dedicated Previous/Next row directly below book identity/series metadata. Show equal-width outlined controls with arrow, adjacent `#index`, and a single-line title; keep unavailable edges stable and disabled as Start/End of series. Use immediate cached neighbors, load the complete series only while online, and replace the selected book within the existing detail screen. Physical-device responsive, long-title, loading, and offline validation remains pending.
- [x] Replace the old app Dark option with one flat ordered list: Follow system, Light, Charcoal, Warm black, OLED black. Charcoal is neutral near-black, Warm black uses warm dark surfaces, and OLED black uses a true-black background; all retain blue/amber accents and explicit dark error containers. Follow system maps to Light/Charcoal, explicit modes ignore system changes, and legacy Dark migrates to Charcoal. Physical-device contrast/readability, OLED, system-switching, dialog/card, and status/navigation-bar validation remains pending.
- [x] Put Change server with a swap icon immediately above Log out/Sign in in the profile menu. Open a current-URL-prefilled editor that retains malformed input and its validation message. Valid submission shows a Cancel/Continue warning that changing server logs out and cancels active downloads; Cancel returns to the populated editor, while Continue clears old server/session/transient state and opens either Login with the normalized reachable URL and Server changed message or Server Setup with the attempted unreachable URL/error. Physical-device interaction and session-transition validation remains pending.
- [x] Add a profile Achievements destination with an overall earned/available summary and adaptive cards split into Unlocked then Locked. Show award dates for earned cards and current/threshold progress for locked cards only when both values are supplied; preserve censored secret fields. Show an older-server unsupported state for HTTP 404 and retry for other failures. Physical-device/server and responsive-layout validation remains pending.

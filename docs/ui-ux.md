...
The functional prototype is stable enough for UI/UX work to continue, but planned UI work remains behind EPUB image compatibility. Version `0.2.1` adds an isolated Readium Kotlin Toolkit 3.0.0 proof for EPUB Preview only; normal Read stays on the custom reader, and comic/audiobook behavior is unchanged. Automated coverage passes, and emulator screenshots visually confirm covers for all four supplied samples. The Samsung Galaxy S24 Preview retest must succeed before deciding on a wider migration. Comic/audiobook work remains later.
...
### Checkpoint 4: EPUB reader - blocked on Readium Preview device verification
...
Implementation candidate: normal EPUB Read follows Komga's paginated interaction pattern and retains the existing custom renderer and controls. EPUB Preview now supplies the staged Readium navigator proof from the same prepared local/temporary file, without progress writes or active-reader replacement. The proof intentionally has no final Lagrange reader chrome yet. Physical S24 verification of the four supplied books is required before deciding whether Readium should replace more of the normal reader or before continuing the planned detail row, lightweight reader chrome, and tutorial.
...
- [x] Add an isolated Readium 3.0.0 EPUB Preview activity while retaining the normal custom reader fallback. Routing and generated bitmap-only SVG-cover tests pass; the full gate passes 216 JVM tests across 36 suites plus lint and both APK assemblies. Emulator screenshots visually confirm the four supplied sample covers. S24 Preview confirmation remains required before any wider migration.
...
1. [ ] Install the new APK on the Samsung Galaxy S24 and Preview `your name.`, Overlord Vol. 1, Overlord Vol. 5, and the no-SVG `Zero Damage Sword Saint Volume 4` control. Confirm covers and later inline images render, Back returns to details, and Preview remains isolated from normal progress. Normal Read deliberately remains the custom fallback. Confirm CBZ/CBR and audiobook routing is unchanged, then decide whether to widen the Readium migration. Keep every later item blocked until this passes.

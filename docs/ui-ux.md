...
Automated gate: 262 JVM tests across 44 suites, lint, debug APK assembly, and Android-test APK assembly pass. Tutorial readability, Series grouping/persistence, section label/divider layout, profile-menu ordering, and hardened audiobook opening/playback are target-device validated; mixed-library cover geometry, audio relaunch, authenticated Preview, the revised audio layout, icon/divider/About, and accessibility checks remain open on device.
...
- [x] Audit and encode one explicit `ReadiumPublicationRoute` matrix: EPUB/KEPUB direct EPUB, PDF direct target, supported audio direct, CBZ direct image, CBR/CB7 cached-CBZ normalization, and MOBI/AZW/AZW3/FB2 EPUB normalization. Normalization-required ebooks now remain `MediaKind.UNKNOWN` instead of falsely opening as EPUB. Focused helper/update tests pass; PDF migration and ebook conversion remain pending and must not be presented as supported yet.

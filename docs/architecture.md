...
Book details are enriched on demand with descriptive, creator, publication, identifier, genre/tag, and file metadata. Their actions render in a horizontally lazy row of compact labeled tiles; selective icons support scanning without replacing the action text, and download/cancel/delete availability still derives from the existing coordinator and offline state. Genre and Tag values use separate wrapping groups, while publication, identifier, and library/file fields are grouped into compact metadata cards that disappear when empty. The series eyebrow sends the existing series id/name key into browser-local navigation, so tapping it replaces the selected book with the normal Series detail destination. The cover opens a full-screen Compose `Dialog` that reuses the authenticated/cache-aware cover loader, decodes away from the main thread, and dismisses from a tap on the cover or the dialog's Android Back handling. Series details no longer depend on the current shelf page: they request the server's ordered series page, show completion and possible gaps, and optionally use the first book synopsis as series context. Hardware and top-bar Back preserve the series destination when a book was opened from within it.

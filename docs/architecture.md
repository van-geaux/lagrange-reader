...
Book details are enriched on demand with descriptive, creator, publication, identifier, genre/tag, and file metadata. Read and Preview show text beside clear Play/Visibility icons; Download uses a clear download icon, while active downloads expose per-file byte progress, percentage/linear or indeterminate status, cancel guidance, and retry failure state. Completed or authentication-interrupted downloads clear their active state. Genre chips open a fully paginated, selected-library server-filtered Books list; Series-detail genre chips open a fully paginated genre-filtered Series catalog. Tags remain informational. The series eyebrow sends the existing series id/name key into browser-local navigation, and the cover opens a full-screen Compose `Dialog` dismissible from any screen tap or Android Back. Library grids support multi-book selection with bulk read/unread actions; the header avoids overlap and stale selections are pruned when the displayed catalog changes.

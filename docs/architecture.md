...
- The native grouped Options screen persists Interface settings for locking the current orientation, suppressing supported haptics, selecting Follow system/Light/Dark app theme, choosing the fresh-start destination (Home/Library/Local books), and reducing motion via immediate catalog jumps. Data settings persist Always/Never/Ask cellular-download policy with browser-wide start/prompt/block behavior, downloaded/disposable-cache size display and confirmed Clear cache that preserves downloads and downloaded-book metadata, Any network/Wi-Fi only/Disabled policy for current scheduled cover work and future metadata work, and default-on confirmation before local-copy deletion. Orientation, opening screen, Reduce motion, cellular behavior, cache clearing, and delete confirmation are device-validated; haptic perceptibility, theme, and background-network-policy validation remain open.

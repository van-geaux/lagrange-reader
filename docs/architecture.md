...
- Home's `Recently added series` preview and `See all` path load the complete `/api/v1/series` catalog with `sort=lastAddedAt` and `order=desc`; Home caps only the preview projection. Series entries are deduplicated by stable series ID and retain `SeriesSummary` identity, name, and cover. If the complete server request is unavailable, the existing book-derived grouping remains an offline/request fallback.

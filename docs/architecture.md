...
- The native grouped Options screen persists Interface settings for locking the current orientation, enabling supported haptics, selecting Follow system/Light/Dark app theme, choosing the fresh-start destination (Home/Library/Local books), and reducing motion via immediate catalog jumps. `MainActivity` installs a preference-aware Compose `LocalHapticFeedback`: enabled requests call `View.performHapticFeedback` with `HapticFeedbackConstants.CONFIRM` on API 30+ or `VIRTUAL_KEY` on API 26-29, leaving Android's system haptic policy authoritative; disabled mode supplies a no-op. The off-to-on transition invokes the new provider immediately, while the on-to-off action receives its pre-change feedback before later requests are suppressed. Current request sites remain Options switch/selection rows and supported Foundation long presses; broader interaction coverage is not yet defined. Data settings persist Always/Never/Ask cellular-download policy with browser-wide start/prompt/block behavior, downloaded/disposable-cache size display and confirmed Clear cache that preserves downloads and downloaded-book metadata, Any network/Wi-Fi only/Disabled policy for current scheduled cover work and future metadata work, and default-on confirmation before local-copy deletion. Orientation, opening screen, Reduce motion, cellular behavior, cache clearing, and delete confirmation are device-validated; the haptic fix, theme, and background-network-policy validation remain open.
...
- The profile Change server flow starts from `BrowserState.serverUrl` and keeps malformed edits in place with the standard validation error. After valid input and explicit confirmation, `AppCoordinator.changeServer` cancels/resets transient work, disables cached-login fallback, and calls `repository.clearServer()` to remove the old configured URL, selected library, caches, session/cookies, current reader, and cover warming before checking the replacement. A reachable replacement is normalized, persisted, and shown prefilled on Login with a Server changed message. If validation fails, the old state remains cleared and Server Setup opens with the attempted URL and error prefilled. Canceling confirmation returns to the populated editor without changing server state.
...
- Comic inference recognizes bare `cbz`, `cbr`, and `cb7` BookOrbit format tokens in addition to dotted extensions and generic comic hints. Remote comics use authenticated `GET /api/v1/cbz/files/{fileId}/pages` for the page count and `GET /api/v1/cbz/files/{fileId}/pages/{pageIndex}` for each rendered image; page changes use the normal comic progress queue. Local ZIP comics are extracted and rendered on-device, including archives whose extension is mislabeled. BookOrbit's server also detects ZIP/RAR content by magic bytes and serves CBZ/CBR/CB7 through the same endpoints. This build deliberately adds no client RAR/7z dependency: downloaded/local CBR and CB7 remain valid but require connectivity for server-side page extraction. `MediaKind.COMIC` bypasses the normal reader Scaffold/top bar and renders a fitted image on a black fullscreen surface with an always-visible page footer. Outer-quarter taps and horizontal swipes move one page; the center half opens a dark rounded options sheet containing title/page, Continue reading, Close book, and a page-position slider. Continue reading, an exposed-content scrim tap, or Android Back dismisses the sheet first; Android Back exits only when the sheet is closed. Remote and local sources share this presentation and the existing persisted progress path. General comic reading is target-device validated; the exhaustive online/offline per-format source matrix remains open.
...
Library Browse and Series catalog grids share conditional end-padding geometry with the overlaid title-initial jump rail. When the 20 dp rail is visible, the grid reserves 32 dp at the trailing edge: 20 dp for the rail, 4 dp for its screen-edge inset, and 8 dp of separation from card content. When the rail is hidden, the grid keeps the normal 16 dp end padding and uses the full available width. This prevents trailing cards from measuring or rendering beneath the rail without permanently narrowing non-rail layouts.

Book details are enriched on demand with descriptive, creator, publication, identifier, genre/tag, and file metadata. Read and Preview show text beside clear Play/Visibility icons; Download and Delete local show visible labels with clear icons. When a local copy is version-stale, online details add an Update local action to the existing book-detail action row. Update local is not exposed from offline snapshots, passes through the same Always/Never/Ask cellular gate, and reuses active progress/cancel/retry state with update-specific text. A compact final three-dot action resolves the selected book against live selected-library `books` first and server-wide `homeBooks` as fallback, then exposes exactly one status action: Mark as read for unfinished books or Mark as unread for read/completed books. It reuses the coordinator's existing status operations and is disabled for offline snapshots. Active downloads expose per-file byte progress, percentage/linear or indeterminate status, cancel guidance, and retry failure state. Completed or authentication-interrupted downloads clear their active state; detail loading is keyed by catalog version/local path and download-active transitions so an open detail refreshes immediately after completion. Successful download/deletion also writes a transient file-path override into `BrowserState`; detail rendering observes the override immediately, and a local-books revision reloads the downloaded list without a browser round trip. Deleting a local copy removes its stale rich-detail cache after the latest cached detail has already served as a metadata fallback for incomplete Local books catalog/snapshot summaries. Genre chips open fully paginated filtered Books/Series lists. Book relation filters follow the official current contract: singular `genre` or `author`, operator `includesAny`, and an array value; this replaces the former `genres`/`contains`/scalar genre request. Tags remain informational and non-clickable because no verified BookOrbit tag filter exists. Library grids support multi-book selection with bulk read/unread actions; the header avoids overlap and stale selections are pruned when the displayed catalog changes.
